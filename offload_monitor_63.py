...
def fetch_roster_staff(date_dir: str, shift: str) -> dict:
    ...
    # DEBUG: show all unique depts found
    all_depts = sorted({e.get('dept','?') for e in on_duty + on_leave})
    print(f"  [roster-debug] depts found: {all_depts}")
...
def _render_manpower_section(roster: dict, supervisor_display: str = "") -> str:
    ...
    excluded_emps = [e for e in on_duty if _is_excluded(e)]
    print(f"  [manpower-debug] on_duty={len(on_duty)}, main={len(main_emps)}, excluded={len(excluded_emps)}")
    print(f"  [manpower-debug] excluded depts: {sorted({e.get('dept','?') for e in excluded_emps})}")
    print(f"  [manpower-debug] main depts: {sorted({e.get('dept','?') for e in main_emps})}")
    by_dept = OrderedDict()
...
(function(){{
  ...
  function fetchFlightInfo(flt, isoDate, cb){{
    /* 1) بحث في الـ JSON المحلي أولاً */
    var local = LOCAL_FLIGHTS[flt.toUpperCase()];
    if(local){{ cb(local); }}

    /* 2) AirLabs */
    if(!AIRLABS_KEY){{ if(!local) cb(null); return; }}
    ...
    function processRow(row){{
      ...
      var result = {{std:std, etd:etd, dest:dest}};
      cb(result);
    }}
  }}
  ...
  document.querySelectorAll('ul[data-flight-list]').forEach(function(ul){{
    ul.querySelectorAll('li').forEach(setupFlightListItem);
    ul._flightList = true;
  }});

  var _origAddListItem = window.addListItem;
  window.addListItem = function(ulId){{
    _origAddListItem(ulId);
    var ul = document.getElementById(ulId);
    if(ul && (ul._flightList || ul.dataset.flightList)){{
      var last = ul.lastElementChild;
      if(last) setupFlightListItem(last);
    }}
  }};
  ...
  function buildSnMap(){{
    var map = {{}};
    document.querySelectorAll('[id^="ul-"] li, .mp-list li').forEach(function(li){{
      var txt = li.innerText.replace(/\u00a0/g,' ').trim();
      /* صيغة: SN82592 Hamed ... أو 82592 Hamed ... */
      var m = txt.match(/^SN?\s*(\d{{4,}})\s+(.+)$/i);
      if(!m) m = txt.match(/^(.+?)\s+(\d{{4,}})$/);
      if(m){{
