...
ROSTER_LOCAL_FILE: Path = DOCS_DIR / "data" / "roster.json"
...
def _download_remote_roster_json() -> dict:
    """Download roster JSON from GitHub raw URL."""
    r = requests.get(
        ROSTER_JSON_RAW,
        timeout=20,
        headers={
    ...
    if not isinstance(data, dict):
        raise ValueError("Remote roster JSON is not a dict")

    days = data.get("days")
    if not isinstance(days, dict):
        raise ValueError("Remote roster JSON missing 'days' dictionary")

    return data



def ensure_local_roster_json(force_refresh: bool = False) -> dict:
    """
    Ensure docs/data/roster.json exists locally.
    Priority:
      1) local cached file
      2) download from remote source and save locally
    """
    if ROSTER_LOCAL_FILE.exists() and not force_refresh:
        try:
            data = json.loads(ROSTER_LOCAL_FILE.read_text(encoding="utf-8"))
            if isinstance(data, dict) and isinstance(data.get("days"), dict):
                return data
        except Exception as exc:
            print(f"  [roster] local roster invalid, refreshing: {exc}")

    try:
        data = _download_remote_roster_json()
        ROSTER_LOCAL_FILE.parent.mkdir(parents=True, exist_ok=True)
        ROSTER_LOCAL_FILE.write_text(
            json.dumps(data, ensure_ascii=False, indent=2),
            encoding="utf-8"
        )
        print(f"  [roster] local roster.json updated from remote")
        return data
    except Exception as exc:
        print(f"  [roster] failed to download remote roster.json: {exc}")

    if ROSTER_LOCAL_FILE.exists():
        try:
            return json.loads(ROSTER_LOCAL_FILE.read_text(encoding="utf-8"))
        except Exception:
            pass
...
def fetch_roster_staff(date_dir: str, shift: str) -> dict:
    """Fetch roster staff directly from the daily roster HTML page.

    The roster site publishes a per-day HTML page at:
      {ROSTER_PAGE_URL}/date/YYYY-MM-DD/

    We parse:
      - department from .deptTitle
      - shift from details.shiftCard[data-shift]
      - employee name/SN from .empRow .empName
    """
    target_shift = shift.strip().lower()
    if not target_shift:
        print(f"  [roster-html] Unknown shift mapping for {shift!r}")
    ...
    try:
        ...
        print(f"  [roster-html] Failed to fetch daily HTML {day_url}: {e}")
        return {"on_duty": [], "on_leave": []}

    try:
        soup = BeautifulSoup(html, "html.parser")
    except Exception as e:
        print(f"  [roster-html] BeautifulSoup error: {e}")
        return {"on_duty": [], "on_leave": []}
    ...
    for dept_card in soup.select(".deptCard"):
        ...
        for shift_card in dept_card.select("details.shiftCard"):
            shift_label = (shift_card.get("data-shift") or "").strip()

            for emp_row in shift_card.select(".empRow"):
                name_el = emp_row.select_one(".empName")
                status_el = emp_row.select_one(".empStatus")
                ...
                if shift_label in _LEAVE_LABELS:
                    item["status"] = shift_label
                    if raw_status:
                        item["remarks"] = raw_status
                    on_leave.append(item)
                elif shift_label.strip().lower() == target_shift:
                    if raw_status:
...
var ROSTER_JSON = "{ROSTER_JSON_RAW}";
...
function parseCardsHtml(html, shift) {{
    var parser = new DOMParser();
    var doc = parser.parseFromString(html, "text/html");
    var target = SHIFT_MAP[shift] || "Morning";
    var onDuty = [];
    doc.querySelectorAll(".deptCard").forEach(function(card) {{
        var dept = (card.querySelector(".deptTitle")||{{}}).textContent || "Unknown";
        dept = dept.trim();
        card.querySelectorAll(".shiftCard").forEach(function(sc) {{
            var label = (sc.querySelector(".shiftLabel")||{{}}).textContent || "";
            label = label.trim();
            sc.querySelectorAll(".empRow").forEach(function(row) {{
                ...
                var name = raw, sn = "";
                var mx = raw.match(/^(.+?)\\s*[-–]\\s*(\\d+)\\s*$/);
                if (mx) {{ name = mx[1].trim(); sn = mx[2].trim(); }}
                if (LEAVE_LABELS.indexOf(label) === -1 && label === target) {{
                    if (EXCLUDED_DEPTS.indexOf(dept.toLowerCase()) === -1 && EXCLUDED_SNS.indexOf(sn) === -1 && name.toLowerCase().indexOf("support") === -1) {{
...
function loadManpower() {{
    ...
    fetch(ROSTER_JSON + "?t=" + Date.now())
        .then(function(r){{ return r.json(); }})
        .then(function(json) {{
            var days = json.days || {{}};
            var day = days[getTodayDate()];
            if (!day || !day.cards_html) {{
                el.innerHTML = '<div class="mp-loading">لا توجد بيانات لهذا اليوم</div>';
                return;
            }}
            var shift = getCurrentShift();
            var emps = parseCardsHtml(day.cards_html, shift);
            if (!emps.length) {{
...
def main() -> None:
    ensure_local_roster_json(force_refresh=False)
