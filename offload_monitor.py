...
def _render_offload_table(flights: list[dict], meta: dict) -> str:
    """Render offload using email-friendly tables so copy/paste to Outlook preserves styling."""
    if not flights:
        return """
    <div style="margin-top:12px;overflow-x:auto;">
      <table width="100%" cellpadding="0" cellspacing="0" border="0"
        ...
        <tr style="background:#0b3a78;">
          <td style="padding:8px 6px;color:#ffffff;font-weight:700;border:1px solid #0a3166;text-align:center;width:30px;">#</td>
          <td style="padding:8px 6px;color:#ffffff;font-weight:700;border:1px solid #0a3166;">AWB</td>
          <td style="padding:8px 6px;color:#ffffff;font-weight:700;border:1px solid #0a3166;text-align:center;">PCS</td>
          <td style="padding:8px 6px;color:#ffffff;font-weight:700;border:1px solid #0a3166;text-align:center;">KGS</td>
          <td style="padding:8px 6px;color:#ffffff;font-weight:700;border:1px solid #0a3166;text-align:center;">Priority</td>
          <td style="padding:8px 6px;color:#ffffff;font-weight:700;border:1px solid #0a3166;">Description</td>
          <td style="padding:8px 6px;color:#ffffff;font-weight:700;border:1px solid #0a3166;text-align:center;">ULD</td>
          <td style="padding:8px 6px;color:#ffffff;font-weight:700;border:1px solid #0a3166;">Offloading Reason</td>
        </tr>
        <tr>
    ...
    for flight in flights:
        ...
        date = flight.get("date", "") or ""
        dest = flight.get("destination", "") or ""
        filename = slugify(f"{flt}_{date}_{dest}") + ".json"
        meta_entry = (meta or {}).get("flights", {}).get(filename, {})
        cards.append(_render_flight(flight, meta_entry))
    return "\n".join(cards)
...
def build_shift_report(date_dir: str, shift: str) -> None:
    ...
    # ── Supervisor name resolution (primary → backup pairs) ──────────
    # Each tuple: (primary_sn, primary_name, backup_sn, backup_name)
    SUPERVISOR_PAIRS = [
        ("80250", "Jay Macalino",          "82445", "Al Muthana Al Shamfari"),
        ("81404", "Mohamed Al Amri",        "81402", "Mohamed Al Siyabi"),
        ("82593", "Mohamed Al Kalbani",     "82550", "Saleh Al Aufi"),
        ("82546", "Saleh Al Rashdi",        "82551", "Omar Al Obaidi"),
    ]

    on_duty_list  = roster.get("on_duty",  [])
    on_leave_list = roster.get("on_leave", [])

    def _find_emp_by_sn(sn, emp_list):
        for e in emp_list:
            if str(e.get("sn", "")).strip() == sn:
                return e
        return None
...
<div style="max-width:900px; margin:12px 10px 30px; display:flex; gap:10px; justify-content:flex-end; flex-wrap:wrap;">
  <button id="btn-copy"
...
(function() {{
  ...
  function buildStandaloneReportHtml() {{
    ...
    var links = clone.querySelectorAll('a[href="../../index.html"]');
    links.forEach(function(a) {{
      var row = a.closest('tr');
      if (row) row.remove();
    }});
    var doc = '<!DOCTYPE html><html><head><meta charset="utf-8"></head>' +
      '<body style="margin:0;padding:20px 0;background:#eef1f7;font-family:Calibri,Arial,sans-serif;">' +
  ...
  async function copyReport() {{
    ...
    var html = buildStandaloneReportHtml();
    if (!html) return;
    var plainEl = document.getElementById('report-content');
    var plain = plainEl ? (plainEl.innerText || plainEl.textContent || '') : '';
    try {{
      ...
      setBtnState(btn, '✅ Copied!', '#059669', false);
    }} catch (e) {{
      console.error(e);
      setBtnState(btn, '❌ Copy failed', '#dc2626', false);
    }}
    setTimeout(function() {{ setBtnState(btn, '📋 Copy Report', '#0b3a78', false); }}, 2500);
  }}

  async function sendEmailNow() {{
    var btn = document.getElementById('btn-email');
    var pin = prompt('أدخل الرقم السري للإرسال');
    if (pin !== '887155') return;
    var defaultRecipients = (localStorage.getItem('email_recipients') || '').trim();
    var recips = prompt('أدخل الإيميلات مفصولة بفاصلة ,\nاتركها فارغة لاستخدام EMAIL_RECIPIENTS من GitHub Actions.', defaultRecipients);
    var recipients = [];
    if (recips && recips.trim()) {{
      recipients = recips.split(',').map(function(x) {{ return x.trim(); }}).filter(Boolean);
      localStorage.setItem('email_recipients', recipients.join(', '));
    }}
    setBtnState(btn, '⏳ Sending…', '#64748b', true);
    var pat = localStorage.getItem('gh_pat');
    if (!pat) {{
      pat = prompt('أدخل GitHub Personal Access Token (repo scope)');
      if (!pat) {{
        setBtnState(btn, '✉️ Send Email Now', '#c2410c', false);
        return;
      }}
      localStorage.setItem('gh_pat', pat);
    }}
    try {{
      var r = await fetch('https://api.github.com/repos/khalidsaif912/offload-monitor/dispatches', {{
        method: 'POST',
        headers: {{
      ...
      if (r.ok || r.status === 204) {{
        setBtnState(btn, '✅ Dispatch sent', '#059669', false);
      }} else {{
        if (r.status === 401 || r.status === 403) localStorage.removeItem('gh_pat');
        setBtnState(btn, '❌ Failed (' + r.status + ')', '#dc2626', false);
      }}
    }} catch (e) {{
      console.error(e);
      setBtnState(btn, '❌ Error', '#dc2626', false);
    }}
    setTimeout(function() {{ setBtnState(btn, '✉️ Send Email Now', '#c2410c', false); }}, 4000);
  }}
...
def send_shift_report_email(date_dir: str, shift: str, recipients_override=None) -> None:
    ...
    if not recipients:
        print("  [email] Skipping — no recipients resolved.")
        return
    ...
    # Keep only the report content in email, remove page-only controls/links.
    html_content = re.sub(r'<tr>\s*<td[^>]*>\s*<a href="../../index\.html".*?</tr>', '', html_content, flags=re.S|re.I)
    html_content = re.sub(r'<!-- ═══ BUTTONS BAR.*?</html>', '</body>\n</html>', html_content, flags=re.S|re.I)
...
def _resolve_force_send_request() -> tuple[str, str, list[str] | None]:
    """Resolve manual-send request from env vars or GitHub repository_dispatch payload."""
    force_date = os.getenv("FORCE_SEND_DATE", "").strip()
    force_shift = os.getenv("FORCE_SEND_SHIFT", "").strip()
    recipients = None

    raw_recips = os.getenv("FORCE_SEND_RECIPIENTS", "").strip()
    if raw_recips:
        recipients = [r.strip() for r in raw_recips.split(",") if r.strip()]

    if force_date and force_shift:
        return force_date, force_shift, recipients
    ...
    try:
        ...
        print(f"[force-send] Could not read GITHUB_EVENT_PATH: {exc}")
        return "", "", recipients

    client_payload = payload.get("client_payload") or {}
    event_action = str(payload.get("action") or payload.get("event_type") or "").strip().lower()
    if not client_payload:
        return "", "", recipients

    force_date = str(client_payload.get("date_dir") or client_payload.get("date") or "").strip()
    force_shift = str(client_payload.get("shift") or "").strip()
    cp_recipients = client_payload.get("recipients")
    if cp_recipients:
        if isinstance(cp_recipients, str):
            recipients = [r.strip() for r in cp_recipients.split(",") if r.strip()]
        elif isinstance(cp_recipients, list):
            recipients = [str(r).strip() for r in cp_recipients if str(r).strip()]

    if force_date and force_shift:
        print(f"[force-send] Resolved from event payload ({event_action or 'repository_dispatch'})")
    return force_date, force_shift, recipients
...
def main() -> None:
    ...
    force_date, force_shift, force_recipients = _resolve_force_send_request()
    print(f"[debug] FORCE_SEND_DATE={force_date!r}  FORCE_SEND_SHIFT={force_shift!r}  FORCE_SEND_RECIPIENTS={force_recipients!r}")
    if force_date and force_shift:
        ...
        if sent_file.exists():
            sent_file.unlink()
        send_shift_report_email(force_date, force_shift, recipients_override=force_recipients)
        sent_file.write_text(now.isoformat(), encoding="utf-8")
