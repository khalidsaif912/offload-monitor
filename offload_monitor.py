...
def fetch_roster_staff(date_dir: str, shift: str) -> dict:
    ...
    # DEBUG: أظهر الأقسام والـ shift labels الموجودة
    all_depts_duty  = sorted({e.get('dept','?') for e in on_duty})
    all_depts_leave = sorted({e.get('dept','?') for e in on_leave})
    print(f"  [roster-debug] ON DUTY depts : {all_depts_duty}")
    print(f"  [roster-debug] ON LEAVE depts: {all_depts_leave}")
...
(function(){{
  ...
  function _attachLiEvents(li, ulRef){{
    /* ulRef مُمرَّر صراحةً لأن li قد لا تكون في DOM بعد */
    function getUl(){{ return ulRef || li.parentElement; }}
    ...
    li.addEventListener('keydown', function(e){{
      if(e.key === 'Enter'){{
        ...
        _attachLiEvents(newLi, parentUl);
        newLi.focus();
        var r = document.createRange();
    ...
    /* flight-list autocomplete */
    var ul = getUl();
    if(ul && ul.getAttribute('data-flight-list')){{
      if(typeof setupFlightListItem === 'function') setupFlightListItem(li);
    }}
    /* MANPOWER autocomplete */
    if(ul && ul.id && (
        ul.id.indexOf('ul-dept-')===0 ||
        ['ul-supervisors','ul-ctu','ul-inventory','ul-support',
...
(function(){{
  ...
  /* صيغة 10APR فقط (بدون سنة) */
  function todayShort(){{
    var f = todayFull();
    return f.slice(0,5); /* 10APR */
  }}

  var FLT_RE = /^([A-Z]{{2,3}})(\d{{1,4}})$/i;

  /* JSON محلي كـ fallback للرحلات من مسقط */
  var LOCAL_FLIGHTS = {{}};
  try{{
    var lf = window._LOCAL_MCT_FLIGHTS;
    if(lf) LOCAL_FLIGHTS = lf;
  }}catch(ex){{}}

  /* ── ghost suggestion ── */
  function showGhost(el, txt){{
    var g = el.querySelector('.ghost-tip') || (el.parentElement && el.parentElement.querySelector(':scope > .ghost-tip'));
    if(!g){{
      g = document.createElement('span');
      g.className = 'ghost-tip';
      g.style.cssText = 'color:#94a3b8;font-style:italic;font-size:11px;display:block;margin-top:1px;pointer-events:none;user-select:none;white-space:nowrap;';
      if(el.tagName === 'TD'){{ el.appendChild(g); }}
      else{{ el.parentElement && el.parentElement.insertBefore(g, el.nextSibling); }}
    }}
  ...
  function forceUpper(el){{
    var sel = window.getSelection();
    var off = sel.rangeCount ? sel.getRangeAt(0).startOffset : 0;
    ...
    if(old !== up){{
      ...
      try{{
        ...
        var nd  = el.firstChild || el;
        rng.setStart(nd, Math.min(off, (nd.textContent||'').length));
        rng.collapse(true);
  ...
  /* ── AirLabs fetch (schedules → flights fallback) ── */
  function fetchFlightInfo(flt, isoDate, cb){{
    var local = LOCAL_FLIGHTS[flt.toUpperCase()];

    /* إذا وجدناها محلياً واكتملت → استخدمها فوراً بدون شبكة */
    if(local && local.dest && local.std){{
      cb(local);
      return;
    }}

    if(!AIRLABS_KEY){{ cb(local||null); return; }}
      ...
      .then(function(j){{
        ...
        if(!rows.length){{
          return fetch('https://airlabs.co/api/v9/flights?api_key='+AIRLABS_KEY
            ...
            .then(function(j2){{
              rows = j2.response || j2.data || [];
              if(!rows.length){{ cb(local||null); return; }}
              processRow(rows[0]);
            }});
        }}
        processRow(rows[0]);
      }})
      .catch(function(){{ cb(local||null); }});

    function processRow(row){{
      var std  = (row.dep_scheduled||row.dep_time||'').slice(11,16);
      var etd  = (row.dep_estimated||'').slice(11,16);
      var dest = (row.arr_iata||'').toUpperCase();
      cb({{std:std, etd:etd, dest:dest}});
    }}
  }}

  /* ════════════════════════════════════
     A) Load Plan & Advance Loading
        أحرف كبيرة + ghost WY101/LHR/10APR2026
     ════════════════════════════════════ */
  function setupFlightListItem(li){{
    var _ghost = '';

    li.addEventListener('input', function(){{
      var txt = forceUpper(li).replace(/\s+/g,'');
      if(!FLT_RE.test(txt)){{ removeGhost(li); _ghost=''; return; }}

      var flt = txt;
      showGhost(li, flt+'/???/'+todayFull()+' — fetching…');

      fetchFlightInfo(flt, todayISO(), function(info){{
        var dest = (info&&info.dest) ? info.dest : '???';
        _ghost = flt+'/'+dest+'/'+todayFull()+' \u2014 Completed as per plan';
        showGhost(li, _ghost);
    ...
    li.addEventListener('keydown', function(e){{
      if(e.key==='Tab' && li.dataset.ghostVal){{
        e.preventDefault();
        li.innerText = li.dataset.ghostVal;
        li.dataset.ghostVal='';
        removeGhost(li);
        /* cursor آخر السطر */
        var r=document.createRange(); r.selectNodeContents(li); r.collapse(false);
        window.getSelection().removeAllRanges(); window.getSelection().addRange(r);
        if(typeof triggerAutosave==='function') triggerAutosave();
      }}
    }});

    li.addEventListener('blur', function(){{ removeGhost(li); }});
  }}

  document.querySelectorAll('ul[data-flight-list]').forEach(function(ul){{
    ul.querySelectorAll('li').forEach(function(li){{ setupFlightListItem(li); }});
    new MutationObserver(function(muts){{
      muts.forEach(function(m){{
        m.addedNodes.forEach(function(n){{
          if(n.tagName === 'LI') setupFlightListItem(n);
        }});
      }});
  ...
  /* ════════════════════════════════════
     B) جدول الأوفلود
        date → تاريخ اليوم تلقائي
        flight → STD/ETD + DEST من AirLabs
     ════════════════════════════════════ */
  function setupTableCell(td){{
    ...
    if(col==='date'){{
      /* عند click أو focus في خلية فارغة → ضع تاريخ اليوم مباشرة */
      function autoDate(){{
      ...
      td.addEventListener('input', function(){{ forceUpper(td); removeGhost(td); }});
    }}

    if(col==='flight'){{
      td.addEventListener('input', function(){{
        var flt = forceUpper(td).replace(/\s+/g,'');
        if(!FLT_RE.test(flt)) return;
        ...
        fetchFlightInfo(flt, isoDate, function(info){{
          removeGhost(td);
          if(!info) return;
          if(stdCell && !stdCell.innerText.replace(/\u00a0/g,'').trim()){{
            var std = info.std || '';
            var etd = info.etd || '';
  ...
  /* ════════════════════════════════════
     C) MANPOWER — تنسيق SN + Autocomplete
        عند كتابة رقم الوظيفة تنبأ بالاسم
     ════════════════════════════════════ */

  /* قاموس SN → Name من الروستر المحمّل في الصفحة */
  function buildSnMap(){{
    var map = {{}};
    /* أولاً: من data-sn/data-name (الأدق) */
    document.querySelectorAll('[data-sn][data-name]').forEach(function(el){{
      var sn   = (el.dataset.sn   || '').replace(/[^0-9]/g,'');
      var name = (el.dataset.name || '').trim();
      if(sn && name) map[sn] = name;
    }});
    /* ثانياً: من نص li — صيغة: SN82592 Ahmed Al Hadi */
    document.querySelectorAll('[id^="ul-"] li').forEach(function(li){{
      var txt = li.innerText.replace(/\u00a0/g,' ').trim();
      var m = txt.match(/^SN\s*(\d{{4,}})\s+(.+)$/i);
      if(m){{
        var sn = m[1].replace(/[^0-9]/g,'');
        var name = m[2].trim();
  ...
  /* أعد بناء الـ SN map بعد تحميل الصفحة */
  var snMap = {{}};
  window.addEventListener('load', function(){{ snMap = buildSnMap(); }});
  setTimeout(function(){{ snMap = buildSnMap(); }}, 1500);

  /* ربط خلايا الجدول الموجودة + مراقبة الإضافة الديناميكية */
  document.querySelectorAll('[data-col]').forEach(setupTableCell);
  /* مراقبة صفوف الجدول الجديدة */
  document.querySelectorAll('table.offload-table tbody, tbody').forEach(function(tbody){{
    new MutationObserver(function(muts){{
      muts.forEach(function(m){{
        m.addedNodes.forEach(function(n){{
          if(n.tagName === 'TR'){{
  ...
  /* Dropdown للـ SN autocomplete */
  function showSnDropdown(li, matches){{
    var old = li.querySelector('.sn-drop');
    if(old) old.remove();
    if(!matches.length) return;

    var dd = document.createElement('div');
    dd.className = 'sn-drop';
    dd.style.cssText = 'position:absolute;left:0;top:100%;background:#fff;border:1px solid #0b3a78;'
                     + 'border-radius:6px;box-shadow:0 4px 16px rgba(11,58,120,.18);z-index:9999;'
                     + 'min-width:240px;max-height:200px;overflow-y:auto;font-size:13px;';

    matches.forEach(function(item){{
      var opt = document.createElement('div');
      opt.style.cssText = 'padding:7px 12px;cursor:pointer;display:flex;gap:10px;align-items:center;';
      opt.innerHTML = '<span style="font-weight:700;color:#0b3a78;min-width:60px;">SN'+item.sn+'</span>'
                    + '<span style="color:#1b1f2a;">'+item.name+'</span>';
      ...
      opt.onmousedown  = function(ev){{
        ev.preventDefault();
        li.innerText = 'SN'+item.sn+' '+item.name;
        dd.remove();
        var r=document.createRange(); r.selectNodeContents(li); r.collapse(false);
        window.getSelection().removeAllRanges(); window.getSelection().addRange(r);
        if(typeof triggerAutosave==='function') triggerAutosave();
  ...
  function setupManpowerLi(li){{
    li.addEventListener('input', function(){{
      ...
      /* اكتشف إذا المستخدم يكتب رقم وظيفة */
      var snMatch = txt.match(/^(?:SN\s*)?(\d+)$/i);
      if(!snMatch) return;

      var typed = snMatch[1];
      var matches = [];
      Object.keys(snMap).forEach(function(sn){{
        if(sn.indexOf(typed)===0) matches.push({{sn:sn, name:snMap[sn]}});
      }});
      /* أيضاً ابحث في بيانات الروستر الكاملة */
      document.querySelectorAll('[data-sn]').forEach(function(el){{
        var sn   = el.dataset.sn   || '';
        var name = el.dataset.name || '';
        if(sn.indexOf(typed)===0 && !matches.find(function(m){{return m.sn===sn;}}))
    ...
    li.addEventListener('blur', function(){{
      setTimeout(function(){{
        var dd=li.querySelector('.sn-drop'); if(dd) dd.remove();
      }}, 200);
    }});
  }}

  /* ربط كل قوائم MANPOWER */
  ['ul-supervisors','ul-ctu','ul-inventory','ul-support','ul-sickleave',
   'ul-annualleave','ul-trainee','ul-overtime'].forEach(function(id){{
    var ul = document.getElementById(id);
    if(!ul) return;
  ...
  /* كذلك الأقسام الديناميكية من الروستر */
  ...
  /* ════════════════════════════════════
     D) D) Sick Leave — autocomplete بالاسم من الروستر
     ════════════════════════════════════ */
  function getAllStaff(){{
    var list = [];
    document.querySelectorAll('.mp-list li').forEach(function(li){{
      var txt = li.innerText.replace(/\u00a0/g,' ').trim();
      /* SN82592 Hamed Al Fulaiti */
      var m = txt.match(/^SN(\d+)\s+(.+)$/i) || txt.match(/^(.+?)\s+(\d{{4,}})$/);
      if(m){{
        var sn   = (m[1]||'').replace(/[^0-9]/g,'');
        var name = (m[2]||'').trim();
        if(name && name.length>2) list.push({{sn:sn,name:name,full:txt}});
      }}
    }});
    return list;
  }}

  var ulSick = document.getElementById('ul-sickleave');
  if(ulSick){{
    function attachSick(li){{
      var dd=null;
      li.addEventListener('input', function(){{
        var txt=li.innerText.replace(/\u00a0/g,' ').trim().toLowerCase();
        if(dd){{dd.remove();dd=null;}}
        if(!txt||txt.length<2) return;
        var staff=getAllStaff().filter(function(s){{
          return s.name.toLowerCase().indexOf(txt)===0||s.full.toLowerCase().indexOf(txt)===0;
        }});
        if(!staff.length) return;
        dd=document.createElement('div');
        dd.style.cssText='position:absolute;background:#fff;border:1px solid #0b3a78;border-radius:6px;'
                        +'box-shadow:0 4px 16px rgba(0,0,0,.15);z-index:9999;min-width:240px;max-height:180px;overflow-y:auto;font-size:13px;';
        staff.forEach(function(s){{
          var opt=document.createElement('div');
          opt.innerHTML='<span style="font-weight:700;color:#0b3a78;min-width:60px;display:inline-block;">SN'+s.sn+'</span> '+s.name;
          opt.style.cssText='padding:7px 12px;cursor:pointer;';
          opt.onmouseenter=function(){{opt.style.background='#eef3fc';}};
          opt.onmouseleave=function(){{opt.style.background='';}};
          opt.onmousedown=function(ev){{
            ev.preventDefault();
            li.innerText='SN'+s.sn+' '+s.name;
            if(dd){{dd.remove();dd=null;}}
            if(typeof triggerAutosave==='function') triggerAutosave();
          }};
          dd.appendChild(opt);
      ...
      li.addEventListener('blur',function(){{setTimeout(function(){{if(dd){{dd.remove();dd=null;}}}},200);}});
    }}
    ulSick.querySelectorAll('li').forEach(attachSick);
    new MutationObserver(function(muts){{
      muts.forEach(function(m){{m.addedNodes.forEach(function(n){{if(n.tagName==='LI')attachSick(n);}});}});
    }}).observe(ulSick,{{childList:true}});
  }}
...
function loadManpower() {{
    ...
    fetch(url)
        ...
        .then(function(htmlText) {{
            ...
            var allEmps = parseRosterHtml(htmlText, shift);
            ...
            if (inventoryEmps.length) {{
                html += '<div class="mp-dept">Inventory</div>';
                inventoryEmps.forEach(function(e) {{ html += buildEmpRow(e.name, e.sn); }});
            }}

            if (supportEmps.length) {{
                html += '<div class="mp-dept">C) Support Team</div>';
                supportEmps.forEach(function(e) {{ html += buildEmpRow(e.name, e.sn); }});
            }}
