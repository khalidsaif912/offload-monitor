...
def _render_manpower_section(roster: dict, supervisor_display: str = "") -> str:
    ...
    # ── Inventory: موظفو الـ Inventory من الروستر (SN 82592 و 990737) ──
    _INVENTORY_SNS = {"82592", "990737"}
    _inventory_from_roster = [e for e in on_duty if str(e.get("sn","")).strip() in _INVENTORY_SNS]
    ...
    # ── C) Support Team: موظفو السبورت من الروستر (SN 82653 و 82565) ──
    _SUPPORT_SNS = {"82653", "82565"}
    _support_from_roster = [e for e in on_duty if _is_support(e)]
    _support_by_sn = [e for e in on_duty if str(e.get("sn","")).strip() in _SUPPORT_SNS]
    # دمج: موظفو support من الروستر + الموظفون المحددون بـ SN
    _seen_sns = set()
    _combined_support = []
    for e in _support_from_roster + _support_by_sn:
        sn_key = str(e.get("sn","")).strip()
        if sn_key not in _seen_sns:
            _seen_sns.add(sn_key)
    ...
    section_inventory = (
        ...
        f'<ul id="ul-inventory" style="{ul_style}">{_inventory_staff_items}</ul>'
