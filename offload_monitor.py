...
def _render_offload_table(flights: list[dict], meta: dict) -> str:
    """Render offload section: per-flight card with header + ops row + cargo table.
    Outlook-copy safe: dark text on light backgrounds.
    """
    if not flights:
        flights = []

    # ── Styles (Outlook-safe: dark text on light backgrounds) ──
    hdr_bg      = "#d6e4f7"
    hdr_color   = "#0b3a78"
    hdr_border  = "#a8c4e0"
    col_hdr_bg  = "#e2ecf7"
    ...
    for flight in flights:
        ...
        meta_entry = meta.get("flights", {})
        # Find matching meta entry
        upd_at = ""
        updates = 0
        for k, v in meta_entry.items():
            if isinstance(v, dict) and v.get("flight", "") == flt:
                upd_at = (v.get("updated_at", "") or "")[:16].replace("T", " ")
                updates = int(v.get("updates", 0))
                break

        # Ops status — derive email received time from saved_at if not set
        ...
        if not email:
            ...
            if _saved_at:
                try:
                    ...
                    email = ""
        physical = (flight.get("physical")   or "").strip() or ""
        cms      = (flight.get("cms")        or "").strip() or ""
        verified = (flight.get("trolley")    or "").strip() or ""
        remarks  = (flight.get("remarks")    or "").strip() or "—"

        upd_label = f'Updated: <strong style="color:#0b3a78;">{upd_at}</strong>' if upd_at else ""

        # ── Helper for header/ops grid cells ──
        _hdr_label_s = (f'padding:7px 8px; background-color:{col_hdr_bg}; color:{col_hdr_clr};'
                        f'font-weight:700; font-size:11.5px; font-family:Calibri,Arial,sans-serif;'
                        f'border:1px solid {hdr_border}; white-space:nowrap;')
        _hdr_val_s   = (f'padding:7px 8px; background-color:{hdr_bg}; color:{hdr_color};'
                        f'font-size:12.5px; font-family:Calibri,Arial,sans-serif;'
                        f'border:1px solid {hdr_border}; font-weight:700;')
        _ops_label_s = (f'padding:7px 8px; background-color:{col_hdr_bg}; color:#8a7560;'
                        f'font-weight:700; font-size:11.5px; font-family:Calibri,Arial,sans-serif;'
                        f'border:1px solid #f0d9be; white-space:nowrap;')
        _ops_val_s   = (f'padding:7px 8px; background-color:#fef6ee; color:#3a2e22;'
                        f'font-size:12px; font-family:Calibri,Arial,sans-serif;'
                        f'border:1px solid #f0d9be; font-weight:700;')
        ...
        # ── Flight header row (merged cells) ──
        _merged_s = (f'padding:7px 10px; background-color:{hdr_bg}; color:{hdr_color};'
                     f'font-size:12.5px; font-family:Calibri,Arial,sans-serif;'
                     f'border:1px solid {hdr_border}; font-weight:700;')
        flight_hdr_table = f"""
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
           style="border-collapse:collapse; font-family:Calibri,Arial,sans-serif; table-layout:fixed;">
      <tr>
        <td style="{_merged_s} width:25%;" contenteditable="true" tabindex="{_next_ti()}"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">✈ Flight:</span> {flt}</td>
        <td style="{_merged_s} width:25%;" contenteditable="true" tabindex="{_next_ti()}"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">Date:</span> {date}</td>
        <td style="{_merged_s} width:25%;" contenteditable="true" tabindex="{_next_ti()}"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">STD/ETD:</span> {std_display} / {etd_display}</td>
        <td style="{_merged_s} width:25%; color:#b45309;" contenteditable="true" tabindex="{_next_ti()}"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">DEST:</span> {dest}</td>
      </tr>
    </table>"""

        # ── Ops + Remarks row (5 cells, Remarks wider) ──
        ops_row_table = f"""
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
           style="border-collapse:collapse; font-family:Calibri,Arial,sans-serif; table-layout:fixed;">
      <tr>
        <td style="{_merged_s} width:15%;" contenteditable="true" tabindex="{_next_ti()}"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">&#9993; Email:</span> {email}</td>
        <td style="{_merged_s} width:15%;" contenteditable="true" tabindex="{_next_ti()}"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">Ramp:</span> {physical}</td>
        <td style="{_merged_s} width:15%;" contenteditable="true" tabindex="{_next_ti()}"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">CMS:</span> {cms}</td>
        <td style="{_merged_s} width:15%;" contenteditable="true" tabindex="{_next_ti()}"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">Verified:</span> {verified}</td>
        <td style="{_merged_s} width:40%;" contenteditable="true" tabindex="{_next_ti()}"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">Remarks:</span> {remarks}</td>
      </tr>
    </table>"""

        # ── Column headers ──
        def _col(label, extra=""):
            return (f'<td style="padding:7px 8px; background-color:{col_hdr_bg}; color:{col_hdr_clr};'
                    f'font-weight:700; font-size:11.5px; font-family:Calibri,Arial,sans-serif;'
                    f'border:1px solid {hdr_border}; {extra}">{label}</td>')

        col_headers = (
            '<tr>'
            + _col('#', 'width:30px;')
            + _col('AWB:', 'width:140px;')
            + _col('PCS:', 'width:55px;')
            + _col('KGS:', 'width:65px;')
            + _col('Priority:', 'width:70px;')
            + _col('Description:', '')
            + _col('ULD:', 'width:70px;')
            + _col('Offloading Reason:', 'width:150px;')
            + '</tr>'
        )
        ...
        if not items or not any((it.get("awb","") or "").strip() for it in items):
            data_rows = f"""
      <tr>
        <td colspan="8" style="padding:12px 10px; border:1px solid {cell_border};
          ...
          NIL — No offload data recorded.
        </td>
      </tr>"""
        else:
            for idx, it in enumerate([i for i in items if (i.get("awb","") or "").strip()], 1):
                ...
                try: total_pcs += int(str(pcs).replace(",", ""))
                except: pass
                try: total_kgs += float(str(kgs).replace(",", ""))
                except: pass

                badge = (
                    f'<span style="display:inline-block;padding:2px 8px;border-radius:4px;'
                    f'background:{badge_bg};border:1px solid {badge_border};color:{badge_color};'
                    f'font-size:10.5px;font-weight:700;white-space:nowrap;">{reason.upper()}</span>'
                ) if reason else "—"
                ...
                td_c = (f'style="padding:7px 8px;border:1px solid {cell_border};'
                        f'font-size:12px;font-family:Calibri,Arial,sans-serif;color:{text_dark};background:{bg};"')

                data_rows += f"""
      <tr>
        <td {td_c}><strong>{idx}</strong></td>
        <td {td_s}><span style="font-family:Consolas,Courier New,monospace;color:#1e40af;font-weight:600;">{awb}</span></td>
        <td {td_c}><strong>{pcs}</strong></td>
        <td {td_c}>{kgs}</td>
        <td {td_c}>{pri}</td>
        <td {td_s}>{desc}</td>
        <td {td_c}>{uld}</td>
        ...
        kgs_display = f"{total_kgs:g}" if total_kgs else "0"
        totals_row = f"""
      <tr>
        <td colspan="8" style="padding:7px 12px; background-color:{totals_bg}; border:1px solid {cell_border};
            border-top:2px solid {totals_border};
            font-family:Calibri,Arial,sans-serif; font-size:12px; font-weight:700; color:{totals_color};">
          Total Shipments: <strong style="font-size:13px;">{real_shipments}</strong>
          &nbsp;&nbsp;|&nbsp;&nbsp;
          Total PCS: <strong style="font-size:13px;">{total_pcs}</strong>
          &nbsp;&nbsp;|&nbsp;&nbsp;
          Total KGS: <strong style="font-size:13px;">{kgs_display}</strong>
        </td>
      </tr>"""

        # ── 3 empty rows matching AWB columns ──
        _empty_td = (f'style="padding:7px 8px;border:1px solid {cell_border};'
                     f'font-size:12px;font-family:Calibri,Arial,sans-serif;color:{text_dark};background:{row_even};"')
        empty_row = f"""
      <tr>
        ...
        three_empty_rows = empty_row * 3

        cards_html += f"""
    {flight_hdr_table}
    {ops_row_table}
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
           style="border-collapse:collapse; font-family:Calibri,Arial,sans-serif; margin-bottom:14px;">
      {col_headers}
      {data_rows}
      {three_empty_rows}
      {totals_row}
    ...
    # If no flights at all, show a NIL card (same grid structure)
    if not cards_html:
        _nil_merged_s = (f'padding:7px 10px; background-color:{hdr_bg}; color:{hdr_color};'
                        f'font-size:12.5px; font-family:Calibri,Arial,sans-serif;'
                        f'border:1px solid {hdr_border}; font-weight:700;')
        cards_html = f"""
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
           style="border-collapse:collapse; font-family:Calibri,Arial,sans-serif; table-layout:fixed;">
      <tr>
        <td style="{_nil_merged_s} width:25%;"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">✈ Flight:</span> NIL</td>
        <td style="{_nil_merged_s} width:25%;"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">Date:</span> —</td>
        <td style="{_nil_merged_s} width:25%;"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">STD/ETD:</span> — / —</td>
        <td style="{_nil_merged_s} width:25%; color:#b45309;"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">DEST:</span> —</td>
      </tr>
    </table>
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
           style="border-collapse:collapse; font-family:Calibri,Arial,sans-serif; table-layout:fixed;">
      <tr>
        <td style="{_nil_merged_s} width:15%;"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">&#9993; Email:</span> —</td>
        <td style="{_nil_merged_s} width:15%;"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">Ramp:</span> —</td>
        <td style="{_nil_merged_s} width:15%;"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">CMS:</span> —</td>
        <td style="{_nil_merged_s} width:15%;"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">Verified:</span> —</td>
        <td style="{_nil_merged_s} width:40%;"><span style="color:{col_hdr_clr};font-weight:700;font-size:11.5px;">Remarks:</span> —</td>
      </tr>
    </table>
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
           style="border-collapse:collapse; font-family:Calibri,Arial,sans-serif; margin-bottom:14px;">
      <tr><td style="padding:7px 8px;background-color:{col_hdr_bg};color:{col_hdr_clr};font-weight:700;font-size:11.5px;font-family:Calibri,Arial,sans-serif;border:1px solid {hdr_border};width:30px;">#</td><td style="padding:7px 8px;background-color:{col_hdr_bg};color:{col_hdr_clr};font-weight:700;font-size:11.5px;font-family:Calibri,Arial,sans-serif;border:1px solid {hdr_border};width:140px;">AWB:</td><td style="padding:7px 8px;background-color:{col_hdr_bg};color:{col_hdr_clr};font-weight:700;font-size:11.5px;font-family:Calibri,Arial,sans-serif;border:1px solid {hdr_border};width:55px;">PCS:</td><td style="padding:7px 8px;background-color:{col_hdr_bg};color:{col_hdr_clr};font-weight:700;font-size:11.5px;font-family:Calibri,Arial,sans-serif;border:1px solid {hdr_border};width:65px;">KGS:</td><td style="padding:7px 8px;background-color:{col_hdr_bg};color:{col_hdr_clr};font-weight:700;font-size:11.5px;font-family:Calibri,Arial,sans-serif;border:1px solid {hdr_border};width:70px;">Priority:</td><td style="padding:7px 8px;background-color:{col_hdr_bg};color:{col_hdr_clr};font-weight:700;font-size:11.5px;font-family:Calibri,Arial,sans-serif;border:1px solid {hdr_border};">Description:</td><td style="padding:7px 8px;background-color:{col_hdr_bg};color:{col_hdr_clr};font-weight:700;font-size:11.5px;font-family:Calibri,Arial,sans-serif;border:1px solid {hdr_border};width:70px;">ULD:</td><td style="padding:7px 8px;background-color:{col_hdr_bg};color:{col_hdr_clr};font-weight:700;font-size:11.5px;font-family:Calibri,Arial,sans-serif;border:1px solid {hdr_border};width:150px;">Offloading Reason:</td></tr>
      <tr>
        <td colspan="8" style="padding:12px 10px; border:1px solid {cell_border};
            color:{nil_color}; text-align:center; font-style:italic; font-size:12px;
            font-family:Calibri,Arial,sans-serif; background:{row_even};">
      ...
      <tr>
        <td colspan="8" style="padding:7px 12px; background-color:{totals_bg}; border:1px solid {cell_border};
            border-top:2px solid {totals_border};
            font-family:Calibri,Arial,sans-serif; font-size:12px; font-weight:700; color:{totals_color};">
          Total Shipments: <strong style="font-size:13px;">0</strong>
          &nbsp;&nbsp;|&nbsp;&nbsp;
          Total PCS: <strong style="font-size:13px;">0</strong>
          &nbsp;&nbsp;|&nbsp;&nbsp;
          Total KGS: <strong style="font-size:13px;">0</strong>
        </td>
      </tr>
    </table>"""

    return f'<div style="margin-top:12px;">{cards_html}</div>'
        ...
        def _count_matching_flights(folder: Path, report_date: str) -> int:
            ...
            for p in folder.glob("*.json"):
                ...
                try:
                    ...
                    rd = _dt_c.strptime(report_date, "%Y-%m-%d")
                    matched = False
                    for fmt in ("%d%b%y", "%d%b%Y", "%d%b", "%Y-%m-%d", "%d-%b-%y", "%d-%b-%Y"):
                        try:
                            parsed = _dt_c.strptime(fd, fmt)
                            if fmt == "%d%b":
                                parsed = parsed.replace(year=rd.year)
                            if parsed.day == rd.day and parsed.month == rd.month and parsed.year == rd.year:
                                matched = True
...
def filter_flights_already_in_other_shifts(flights: list[dict], now: datetime) -> list[dict]:
    """تمنع تكرار الرحلات: إذا كانت رحلة محفوظة مسبقاً في مناوبة أخرى لنفس اليوم → لا تُحفظ مرة ثانية.

    هذا يحل مشكلة بقاء بيانات المناوبة الصباحية في ملف OneDrive
    وإعادة حفظها في مناوبة الظهر أو الليل.
    """
    date_dir = now.strftime("%Y-%m-%d")
    current_shift = get_shift(now)

    # اجمع أسماء الرحلات المحفوظة في المناوبات الأخرى
    existing_flights = set()
    for shift in ("shift1", "shift2", "shift3"):
        if shift == current_shift:
            continue  # لا نتحقق من المناوبة الحالية
    ...
    for f in flights:
        ...
        if flt_name and flt_name in existing_flights:
            skipped.append(flt_name)
        else:
            kept.append(f)

    if skipped:
        print(f"  [dup-filter] Skipped {len(skipped)} flight(s) already in other shifts: {', '.join(skipped)}")
    print(f"  [dup-filter] Kept {len(kept)} new flight(s) for {current_shift}.")
    return kept
...
def main() -> None:
    ...
    flights = filter_flights_already_in_other_shifts(flights, now)

    if not flights:
        print("WARNING: All flights filtered out by STD — no flights for this shift.")
        STATE_FILE.write_text(new_hash, encoding="utf-8")
        build_root_index(now)
        return
