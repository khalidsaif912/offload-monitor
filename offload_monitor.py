...
def _extract_report_content_html(page_html: str) -> str:
    ...
    Also strips contenteditable and tabindex attributes (not valid in email).
    """
    soup = BeautifulSoup(page_html, "html.parser")
    report = soup.find(id="report-content")
    node = report if report else soup.body
    if not node:
        return page_html

    # Strip attributes that are invalid / harmful in email clients
    for tag in node.find_all(True):
        tag.attrs.pop("contenteditable", None)
        tag.attrs.pop("tabindex", None)
