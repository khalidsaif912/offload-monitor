...
def build_table_body(flights):
    ...
    for fl in flights:
        ...
        # 1) Row 1: flight info (inside the table)
        html += (
            '<tr style="background:#1e3a5f;">'
            f'<td colspan="{NCOLS}" style="padding:10px 14px;'
            'border:1px solid #163259;font-size:13px;font-weight:700;color:#fff;">'
            f'&#9992;&nbsp;<span style="color:#ffd966;">{flight}</span>'
            f'<span style="font-weight:400;color:#b8d0f0;margin-left:16px;">'
            f'Date:&nbsp;{date}&nbsp;&nbsp;|&nbsp;&nbsp;Dest:&nbsp;{dest}'
            '</span></td></tr>'
        )

        # 2) Row 2: column headers (repeat per flight)
        html += (
            '<tr>'
            f'<th {TH}>ITEM</th>'
            f'<th {TH}>DATE</th>'
        ...
        if not ships:
            html += (
                f'<tr><td colspan="{NCOLS}" style="{B}color:#9ca3af;font-style:italic;">'
                'No shipments for this flight.</td></tr>'
            )
            continue

        # Total PCS per flight (put it in "Pieces Verification" on first shipment row only)
        flight_total_pcs = sum(_si(s["pcs"]) for s in ships)

        # Reset item numbering per flight
        idx = 0
        for i, s in enumerate(ships):
            idx += 1
            bg = "#f5f7fb" if i % 2 == 0 else "#ffffff"
            remarks = (
                'AWB:&nbsp;<span style="font-family:Courier New,monospace;'
                f'color:#0b3a78;font-weight:700;">{s["awb"]}</span>'
                f'&nbsp;|&nbsp;PCS:&nbsp;<strong>{s["pcs"]}</strong>'
                f'&nbsp;|&nbsp;KGS:&nbsp;<strong>{s["kgs"]}</strong>'
                + (f'&nbsp;|&nbsp;{s["desc"]}' if s["desc"] else "")
            )
            pieces = str(flight_total_pcs) if idx == 1 else ""
            html += (
                f'<tr style="background:{bg};">'
                + _td(str(idx), "text-align:center;font-weight:700;color:#1b1f2a;"
                      "min-width:38px;background:#e8edf8;")   # ITEM
                + _td(date, "white-space:nowrap;")             # DATE
                + _td(f'<strong style="color:#0b3a78;">{flight}</strong>')  # FLIGHT
                + _td()                                        # STD/ATD
                + _td(f'<strong style="color:#0b3a78;">{dest}</strong>')    # DEST
                + _td()                                        # Email
                + _td()                                        # Physical
                + _td()                                        # Trolley
                + _td()                                        # CMS
                + _td(pieces, "text-align:center;font-weight:700;")         # Pieces
                + _td(s["reason"], "color:#c0392b;font-weight:700;")        # Reason
                + _td(remarks)                                 # Remarks
                + "</tr>"
            )
    return html
...
def build_report_html(flights, generated_at_local, shift_label):
    ...
    return (
        ...
        f'<td colspan="5" style="padding:9px 14px;border:1px solid #c8d0e8;"></td>'
        f'<td style="padding:9px 14px;border:1px solid #c8d0e8;font-weight:700;'
        f'color:#c0392b;font-size:12px;text-align:center;">{total_pcs}&nbsp;pcs</td>'
        f'<td style="padding:9px 14px;border:1px solid #c8d0e8;font-weight:700;'
        f'color:#c0392b;font-size:12px;">{total_kgs:.0f}&nbsp;kgs</td>'
        '</tr></table>'
