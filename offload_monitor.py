...
(function(){{
  ...
  function cleanEditableText(el) {{
    return String((el && el.innerText) || '').replace(/\u00a0/g,' ').replace(/\s+/g,' ').trim();
  }}
  ...
  function collectSnMatches(typed) {{
    ...
    matches.sort(function(a, b) {{
      if(a.sn === typed && b.sn !== typed) return -1;
      if(b.sn === typed && a.sn !== typed) return 1;
  ...
  function setupManpowerLi(li) {{
    ...
    function updateManpowerSuggestion() {{
      closeSnDropdown();
      /* إذا منسق مسبقاً → لا تعيد التنسيق */
      if(li.querySelector && li.querySelector('[data-sn][data-name]')) return;
    ...
    li.addEventListener('paste', function() {{ setTimeout(function() {{ updateManpowerSuggestion(); }}, 30); }});
  ...
  function handleManpowerInteractive(node) {{
    var li = isManpowerLiTarget(node);
    if(!li) return;
    /* إذا كان العنصر منسقاً مسبقاً (يحتوي span[data-sn]) → لا تعيد التنسيق */
    if(li.querySelector && li.querySelector('[data-sn][data-name]')) {{
      closeSnDropdown();
      return;
    }}
  ...
  document.addEventListener('focusin', function(ev) {{
    ...
    if(li) handleManpowerInteractive(li);
  }});
  document.addEventListener('click', function(ev) {{
    var li = isManpowerLiTarget(ev.target);
    if(li) handleManpowerInteractive(li);
  ...
  document.addEventListener('keyup', function(ev) {{
    var li = isManpowerLiTarget(ev.target);
    if(li && !['Enter','Tab','Escape'].includes(ev.key)) handleManpowerInteractive(li);
  }});
  document.addEventListener('paste', function(ev) {{
    var li = isManpowerLiTarget(ev.target);
    if(li) setTimeout(function(){{ handleManpowerInteractive(li); }}, 30);
  }});
