...
def extract_shipments(soup):
    ...
    # البحث عن صف AWB — قد يكون الصف الثاني (بعد FLIGHT# DATE DESTINATION)
    for idx, row in enumerate(rows):
        ...
        if "AWB" in texts:
            ...
            for i, h in enumerate(texts):
                if h == "AWB":                       col_map["awb"]   = i
                elif h == "PCS":                     col_map["pcs"]   = i
                elif h in ("KGS","KG","WEIGHT"):     col_map["kgs"]   = i
    ...
    print(f"  🗂️  col_map: {col_map} | header_row: {header_row_index}")

    # نقرأ الصفوف بعد الهيدر فقط
    ...
    for row in data_rows:
        ...
        if any(k in first for k in ["AWB", "FLIGHT", "TOTAL", "CARGO", "DATE"]):
            continue
        if len([t for t in texts if t.strip()]) < 2:
            continue

        def get(key, texts=texts):
            i = col_map.get(key)
