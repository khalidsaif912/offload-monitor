...
def _airlabs_best_row(rows: list, flight_iata: str, flight_date: str | None) -> dict | None:
    ...
    Priority:
      1. Exact flight_iata match + exact date match in scheduled/estimated/actual fields.
      2. Exact flight_iata match with the richest schedule fields.
      3. Any row as a last fallback.
    """
    if not rows:
        return None
    ...
    def _row_time_blob(row: dict) -> str:
        return " | ".join(
            str(row.get(k) or "")
            for k in (
                "dep_scheduled",
    ...
    if flight_date:
        dated = [row for row in candidates if flight_date in _row_time_blob(row)]
        if len(dated) == 1:
            return dated[0]
        if len(dated) > 1:
            candidates = dated
    ...
    def _score(row: dict) -> tuple[int, int, int]:
        has_sched = 1 if (row.get("dep_scheduled") or "").strip() else 0
        has_est = 1 if (row.get("dep_estimated") or row.get("dep_estimated_utc") or "").strip() else 0
        has_dest = 1 if (row.get("arr_iata") or "").strip() else 0
        return (has_sched, has_est, has_dest)

    best = max(candidates, key=_score)

    if len(candidates) > 1:
        print(
            f"  [AirLabs] INFO: {len(candidates)} candidate rows for {flight_iata} "
            f"date={flight_date!r}; selected best scored row with arr_iata={best.get('arr_iata')!r}."
        )
    return best
...
def fetch_flight_info_airlabs(
    ...
    *,
    ...
    std  = _time_only(str(best.get("dep_scheduled")  or best.get("dep_time") or best.get("dep_time_utc") or ""))
...
def fetch_flight_info_flightradar(
    ...
    *,
    ...
    return {"std": std, "etd": etd, "atd": atd, "dest": dest}
...
def fetch_flight_info_muscatairport(
    ...
    *,
    ...
    print(f"  [MuscatAirport] {flight_iata} → dest={dest!r}, std={std!r}, etd={etd!r}")
    return {"std": std, "etd": etd, "dest": dest}
...
def fetch_flight_info_with_fallbacks(
    ...
    *,
    ...
    merged: dict[str, str] = {"std": "", "etd": "", "dest": ""}
    used_sources: list[str] = []

    for source_name, fn in sources:
        # تحقق هل كل الحقول ممتلئة — إذا نعم، لا داعي لمصادر إضافية
        if all(merged[k].strip() for k in ("std", "dest")):
            break
        ...
        for key in ("std", "etd", "dest"):
            new_val = (info.get(key) or "").strip()
            if new_val and not merged[key].strip():
                merged[key] = new_val
    ...
    if missing:
        print(f"  [⚠ fallback] {flight_iata}: الحقول التالية فارغة بعد جميع المصادر: {', '.join(missing)}")
...
def _render_offload_table(flights: list[dict], meta: dict) -> str:
    ...
    def _to_muscat_time(time_str: str) -> str:
        """AirLabs dep_time is already in airport local time (Muscat = UTC+4).
        For ISO datetimes with explicit tz, convert to Muscat.
        Plain HH:MM from dep_time is already local — return as-is.
        """
        s = (time_str or "").strip()
        if not s:
            return ""
        # If it's a full ISO datetime with timezone info, convert to Muscat
        try:
            ...
            pass
        # Plain HH:MM — already local Muscat time from AirLabs dep_time
        return s
...
def _render_manpower_section(roster: dict, supervisor_display: str = "") -> str:
    ...
    section_b = (
        ...
        f'<ul id="ul-ctu" style="{ul_style}">{_b_items}</ul>'
        f'<button onclick="addListItem(\'ul-ctu\')" style="{_btn_style}">+ Add</button>'
    )

    # section_c — build items outside f-string
    if sup_li:
        _c_items = "".join(
...
def build_shift_report(date_dir: str, shift: str) -> None:
    ...
    # ── Enrich STD/ETD + DEST with fallbacks: AirLabs -> Flightradar -> Muscat Airport ──
    enriched_count = 0
    for f in flights:
        ...
        try:
            ...
            if not info:
                print(f"  [⚠ enrich] {flt}: لم يُعثر على أي بيانات من جميع المصادر الثلاثة (AirLabs, Flightradar, MuscatAirport)")
                continue
            ...
            new_std_etd = current_std_etd
            if std and etd and std != etd:
                new_std_etd = f"{std}|{etd}"
            elif std:
                new_std_etd = std
            elif not current_std_etd and etd:
                new_std_etd = etd
            else:
                print(f"  [⚠ enrich] {flt}: لم يتم العثور على STD موثوق؛ سيتم الإبقاء على القيمة الحالية {current_std_etd!r}")

            if new_std_etd != current_std_etd:
                f["std_etd"] = new_std_etd

            if dest and dest != current_dest:
                f["destination"] = dest
            elif not dest and not current_dest:
                print(f"  [⚠ enrich] {flt}: DEST لا يزال فارغاً رغم البحث في جميع المصادر")

            if ((f.get("std_etd") or "").strip() != current_std_etd) or ((f.get("destination") or "").strip().upper() != current_dest):
                enriched_count += 1
                _update_flight_json(folder, f)
                print(f"  [enrich] {flt} updated via {source_name}")
        except Exception as exc:
            print(f"  [report-enrich] {flt}: {exc}")

    print(f"Enriched {enriched_count} flight(s) via fallback chain.")
