    ...
    def _flight_date_matches(flt_date_str: str, report_date: str) -> bool:
        ...
        for fmt in ("%d%b%y", "%d%b%Y", "%d%b", "%Y-%m-%d", "%d-%b-%y", "%d-%b-%Y"):
            try:
                ...
                # تحقق من التطابق وأرجع النتيجة فقط بعد نجاح الـ parse
                return (parsed.day == rd.day and parsed.month == rd.month and parsed.year == rd.year)
            except ValueError:
                continue
...
<table width="1100" cellpadding="0" cellspacing="0" border="0" id="report-content"
  ...
  <!-- ═══ HEADER ═══ -->
  <tr>
    <td data-email-keep="1" style="background-color:#0b3a78; padding:0;">
      <table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation">
        <tr>
          <td width="8" data-email-keep="1" style="background-color:#f59e0b; font-size:1px; line-height:1px;">&nbsp;</td>
          <td data-email-keep="1" style="padding:20px 22px 18px 16px; background-color:#0b3a78;">
            <table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation">
              <tr>
                <td data-email-keep="1" style="background-color:#0b3a78;">
                ...
                <td align="right" valign="middle" data-email-keep="1" style="padding-right:6px; background-color:#0b3a78;">
...
(function(){{
  ...
  /* ══════════════════════════════════════════════════════════
     buildReportHtml — توليد HTML نظيف للإيميل/النسخ
     المنطق:
       1) نسخ العنصر كما هو (inline styles موجودة بالفعل في HTML)
       2) تحويل الخلفيات الداكنة → فاتحة، ما عدا البنر (data-email-keep)
       3) تنظيف contenteditable / tabindex / back-link
     ══════════════════════════════════════════════════════════ */
  function buildReportHtml(){{
    var el = document.getElementById('report-content');
    if(!el) return null;

    /* ── خطوة 1: نسخ بسيط بدون computed styles ── */
    var clone = el.cloneNode(true);

    /* ── خطوة 2: تحويل الألوان الداكنة → فاتحة (ما عدا البنر) ── */
    ...
    function normColor(s){{ return (s||'').toLowerCase().replace(/\\s/g,''); }}

    var DARK_BG_LIST = [
      'rgb(11,58,120)','#0b3a78',
    ...
    function isDarkBg(s){{
      var n = normColor(s);
      for(var i=0;i<DARK_BG_LIST.length;i++) if(n===normColor(DARK_BG_LIST[i])) return true;
      return false;
    }}
    function isWhiteText(s){{
      var n = normColor(s);
      return n==='#ffffff'||n==='#fff'||n==='rgb(255,255,255)';
    }}

    /* تحويل العناصر خارج البنر */
    clone.querySelectorAll('*').forEach(function(el2){{
      if(isInBanner(el2)) return;
      var bg = el2.style.backgroundColor;
      if(isDarkBg(bg)){{
        el2.style.backgroundColor = '#eef3fc';
        el2.style.color = '#0b3a78';
      }}
      var bgFull = el2.style.background;
      if(bgFull && bgFull.toLowerCase().indexOf('linear-gradient') !== -1){{
        el2.style.background = '';
        el2.style.backgroundColor = '#eef3fc';
        el2.style.color = '#0b3a78';
      }}
      if(isWhiteText(el2.style.color)){{
        el2.style.color = '#1b1f2a';
      }}
    }});

    /* ── خطوة 3: البنر — تثبيت الألوان بشكل قاطع ── */
    clone.querySelectorAll('[data-email-keep]').forEach(function(el2){{
      var bg = normColor(el2.style.backgroundColor);
      if(bg && bg !== normColor('#f59e0b') && bg !== 'rgb(245,158,11)'){{
        el2.style.backgroundColor = '#0b3a78';
      }}
    }});
    var bannerRoot = clone.querySelector('[data-email-keep="1"]');
    if(bannerRoot){{
      bannerRoot.querySelectorAll('div,span,strong,td,th').forEach(function(el2){{
        var c = normColor(el2.style.color);
        if(c === normColor('#fde68a') || c === 'rgb(253,230,138)') return;
        if(c === normColor('#93c5fd') || c === 'rgb(147,197,253)') return;
    ...
    /* ── خطوة 5: تنظيف contenteditable / tabindex / back-link ── */
...
def build_root_index(now: datetime) -> None:
    ...
    for day in day_dirs:
        for shift in ("shift1", "shift2", "shift3"):
            # ابنِ NIL فقط إذا لم يوجد تقرير أصلاً — لا تحذف التقارير الموجودة
            existing_report = DOCS_DIR / day / shift / "index.html"
            ...
            try:
                ...
                for fmt in ("%d%b%y", "%d%b%Y", "%d%b", "%Y-%m-%d", "%d-%b-%y", "%d-%b-%Y"):
                    try:
                        ...
                        if fmt == "%d%b":
                            parsed = parsed.replace(year=rd.year)
                        matched = (parsed.day == rd.day and parsed.month == rd.month and parsed.year == rd.year)
                        break  # وُجد format مناسب، توقف
                    except ValueError:
                        continue
                else:
