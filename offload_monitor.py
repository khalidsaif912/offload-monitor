...
def _render_manpower_section(roster: dict) -> str:
    """Render Section 6 MANPOWER — grouped by department with total staff count."""
    import re as _re
    from collections import OrderedDict
    on_duty = roster.get("on_duty", [])

    td_style = "font-family:Calibri,Arial,sans-serif;font-size:13px;color:#1b1f2a;line-height:1.8;"
    ul_style = "margin:4px 0 14px 22px;padding:0;font-family:Calibri,Arial,sans-serif;font-size:13px;color:#1b1f2a;line-height:1.85;"
    nil_item = '<li style="color:#64748b;">NIL</li>'

    # Dept label normalisation — map roster dept names → display names
    _DEPT_DISPLAY = {
        "supervisors":      "Supervisors",
        "supervisor":       "Supervisors",
        "load control":     "Load Control",
        "loadcontrol":      "Load Control",
        "export checker":   "Export Checkers",
        "export checkers":  "Export Checkers",
        "exportchecker":    "Export Checkers",
        "export operator":  "Export Operators",
        "export operators": "Export Operators",
        "exportoperator":   "Export Operators",
    }

    def _normalise_dept(dept: str) -> str:
        return _DEPT_DISPLAY.get(dept.lower().strip(), dept.strip())

    def _fmt_employee(emp: dict) -> str:
        """Format one employee line: SN XXXXX - Name"""
        raw  = emp.get("name", "").strip()
        sn   = str(emp.get("sn") or "").strip()

        # Handle embedded format: 'Name - 12345 (note)'
        ...
        if m:
            ...
            return f"<li>SN {sn_part} - {name_part}{note_html}</li>"

        sn_html = f"SN {sn} - " if sn else ""
        return f"<li>{sn_html}{raw}</li>"

    if not on_duty:
        no_data = '<li style="color:#64748b;">No roster data available for this date.</li>'
        grouped_html = f'<ul style="{ul_style}">{no_data}</ul>'
    else:
        # Group by normalised department (preserve insertion order)
        by_dept: dict[str, list] = OrderedDict()
        for emp in on_duty:
            dept = _normalise_dept(emp.get("dept", "Unknown"))
            by_dept.setdefault(dept, []).append(emp)

        total_staff = len(on_duty)

        # Update section header to include total count
        # (injected via a special placeholder replaced in build_shift_report — simpler: just embed here)
        grouped_html = f'<p style="font-family:Calibri,Arial,sans-serif;font-size:12px;color:#64748b;margin:0 0 10px 0;">Total Staff: <strong style="color:#0b3a78;">{total_staff}</strong></p>'

        dept_order = ["Supervisors", "Load Control", "Export Checkers", "Export Operators"]
        ...
        for dept in sorted(by_dept.keys(), key=_dept_sort_key):
            emps  = by_dept[dept]
            count = len(emps)
            li_rows = "\n".join(_fmt_employee(e) for e in emps)
            grouped_html += f"""
          <div style="margin-bottom:14px;">
            <div style="font-family:Calibri,Arial,sans-serif;font-size:14px;font-weight:700;
                        color:#0b3a78;margin-bottom:4px;letter-spacing:0.3px;">
              {dept}
              <span style="font-size:12px;font-weight:600;color:#4b6cb7;
                           background:#eef3fc;border:1px solid #c7d7f5;
                           border-radius:10px;padding:1px 8px;margin-left:6px;">{count}</span>
            </div>
            <ul style="{ul_style}">{li_rows}</ul>
          </div>"""

    section_b = f'<strong style="color:#0b3a78;font-weight:700;font-size:13px;">B) CTU Staff On Duty:</strong><ul style="{ul_style}">{nil_item}</ul>'
    section_c = f'<strong style="color:#0b3a78;font-weight:700;font-size:13px;">C) Support Team:</strong><ul style="{ul_style}">{nil_item}</ul>'
    section_d = f'<strong style="color:#0b3a78;font-weight:700;font-size:13px;">D) Sick Leave / No Show / Others:</strong><ul style="{ul_style}">{nil_item}</ul>'
    section_e = f'<strong style="color:#0b3a78;font-weight:700;font-size:13px;">E) Annual Leave / Course / Off in Lieu:</strong><ul style="{ul_style}">{nil_item}</ul>'
    section_f = f'<strong style="color:#0b3a78;font-weight:700;font-size:13px;">F) Trainee:</strong><ul style="{ul_style}">{nil_item}</ul>'
    section_g = f'<strong style="color:#0b3a78;font-weight:700;font-size:13px;">G) Overtime Justification:</strong><ul style="{ul_style}">{nil_item}</ul>'
...
def build_shift_report(date_dir: str, shift: str) -> None:
    ...
    total_staff   = len(roster.get("on_duty", []))
    manpower_hdr  = (
        f'6.  MANPOWER'
        f'<span style="font-size:11px;font-weight:600;color:#4b6cb7;'
        f'background:#dde8fb;border-radius:10px;padding:1px 9px;margin-left:8px;">'
        f'Total Staff: {total_staff}</span>'
        if total_staff else "6.  MANPOWER"
    )
  ...
  <tr><td style="padding:14px 24px 0 24px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      <tr>
        <td width="4" style="background-color:#0b3a78;">&nbsp;</td>
        <td style="padding:6px 10px; background-color:#eef3fc;">
          <span style="font-family:Calibri,Arial,sans-serif; font-size:12px; font-weight:700; color:#0b3a78; letter-spacing:1px;">{manpower_hdr}</span>
        </td>
      </tr>
    </table>
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:10px;">
    ...
    for day in day_dirs:
        ...
        for shift in ("shift1", "shift2", "shift3"):
            ...
            <a class="shift-card" href="{day}/{shift}/">
                ...
                # مناوبة بدون تقرير — NIL
                rows += f"""
            <div class="shift-card" style="opacity:.6;cursor:default;">
                <div class="sc-icon">{meta_s['icon']}</div>
                <div class="sc-body">
                    <div class="sc-title">{meta_s['ar']} <span class="sc-en">/ {meta_s['label']}</span></div>
                    <div class="sc-time">{meta_s['time']}</div>
