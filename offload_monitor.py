...
(function() {{
  ...
  if (copyBtn) {{
    copyBtn.addEventListener('click', function() {{
      ...
      var full = '<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{{background:#eef1f7;font-family:Calibri,Arial,sans-serif;}}</style></head><body style="margin:0;padding:20px 0;background:#eef1f7;">' + html + '</body></html>';
      ...
      function markCopied() {{
        ...
        setTimeout(function() {{
          ...
          copyBtn.style.background = '#0b3a78';
        }}, 2500);
      }}

      navigator.clipboard.writeText(full).then(markCopied).catch(function() {{
...
def _prepare_email_report_html(html_content: str) -> tuple[str, bytes | None, str | None]:
    """Prepare the saved report HTML for email clients.

    - removes web-only controls (Copy / Send buttons + scripts)
    - converts the inline certifications data-URI image into a CID reference
      so it can render reliably in email apps
    """
    import base64

    soup = BeautifulSoup(html_content, "html.parser")

    # Remove any JavaScript — email clients ignore it and some renderers show artifacts.
    ...
    for btn_id in ("btn-copy", "btn-email"):
        ...
        if btn is not None:
            parent_div = btn.find_parent("div")
            if parent_div is not None:
                button_bar = parent_div
            btn.decompose()
    if button_bar is not None:
        button_bar.decompose()

    inline_image_bytes: bytes | None = None
    inline_image_subtype: str | None = None

    # Convert the certifications image from data URI to CID.
    for img in soup.find_all("img"):
        src = (img.get("src") or "").strip()
        alt = (img.get("alt") or "").strip().lower()
        if not src.startswith("data:image/"):
            continue
        if "cert" not in alt and "certification" not in alt:
            continue
        ...
        declared_subtype = m.group(1).lower()
        try:
        ...
        # Detect the real file type from the magic bytes when possible.
        if img_bytes.startswith(b"\xff\xd8\xff"):
            ...
            real_subtype = "jpeg" if declared_subtype in ("jpg", "jpeg") else declared_subtype
        img["src"] = "cid:certifications"
        inline_image_bytes = img_bytes
        inline_image_subtype = real_subtype
        break

    return str(soup), inline_image_bytes, inline_image_subtype
...
def send_shift_report_email(date_dir: str, shift: str) -> None:
    ...
    html_content, inline_image_bytes, inline_image_subtype = _prepare_email_report_html(html_content)
    ...
    if inline_image_bytes and inline_image_subtype:
        img_part = MIMEImage(inline_image_bytes, _subtype=inline_image_subtype)
        img_part.add_header("Content-ID", "<certifications>")
        img_part.add_header(
            "Content-Disposition",
            "inline",
            filename=f"certifications.{inline_image_subtype}",
        )
        msg.attach(img_part)
