...
def _render_manpower_section(roster: dict, supervisor_display: str = "") -> str:
    ...
    if sup_in_roster:
        ...
        if (supervisor_display
            ...
            sup_li_roster += f'<li><strong>{supervisor_display}</strong></li>\n'
        grouped_html += f"""
...
def build_shift_report(date_dir: str, shift: str) -> None:
    ...
    if not duty_supervisor_name:
        for emp in on_duty_list:
            if "supervisor" in emp.get("dept", "").lower():
                duty_supervisor_name = emp.get("name", "").strip()
                if not supervisor_name:
                    supervisor_name = duty_supervisor_name
                break
    ...
    signature_display = duty_supervisor_name if duty_supervisor_name else supervisor_display
...
(function(){{
  ...
  function forceReadableContrast(node){{
    ...
    if(bg === 'rgb(11,58,120)' || bg === '#0b3a78' || bg === 'rgb(30,64,175)' || bg === '#1e40af' || bg === 'rgb(37,99,235)' || bg === '#2563eb'){{
