...
(function(){{
  ...
  function fetchFlightInfo(flt, isoDate, cb){{
    ...
    /* إذا وجدناها محلياً واكتملت البيانات → استخدمها مباشرة بدون شبكة */
    if(local && local.dest && local.std){{
      cb(local);
      return;   /* ← مهم: لا نكمل للشبكة */
    }}

    /* 2) AirLabs — فقط إذا عندنا مفتاح */
    if(!AIRLABS_KEY){{
      cb(local || null);   /* إرجاع المحلي حتى لو ناقص، أو null */
      return;
    }}
  ...
  window.addListItem = function(ulId){{
    ...
    /* لا حاجة لإعادة ربط — MutationObserver يفعل ذلك تلقائياً */
  }};
  ...
  function buildSnMap(){{
    ...
    /* أولاً: من data-sn / data-name attributes (الأدق) */
