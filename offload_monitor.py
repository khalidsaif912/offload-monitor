...
def download_file() -> tuple[str, str, str]:
    """Download the OneDrive file and return (html_text, last_modified_local_str, last_modified_shift_date).

    last_modified_local_str is HH:MM in TIMEZONE, derived from the HTTP
    Last-Modified header. last_modified_shift_date is YYYY-MM-DD in the
    effective shift date derived from the same timestamp. Both fall back to ''
    ...
    if lm_header:
        try:
            ...
            lm_shift_date = get_shift_date(lm_local, get_shift(lm_local))
            print(f"  [OneDrive] Last-Modified: {lm_header} → local: {lm_local.isoformat()} | shift_date: {lm_shift_date}")
        except Exception as exc:
            print(f"  [OneDrive] Failed to parse Last-Modified: {exc}")

    return response.text, lm_str, lm_shift_date
...
def build_shift_report(date_dir: str, shift: str) -> None:
    ...
    if os.environ.get("AIRLABS_API_KEY", "").strip():
        for f in flights:
            ...
            flt = re.sub(r"\s+", "", (f.get("flight") or "").strip().upper())
            if not flt:
                continue
            try:
...
def main() -> None:
    ...
    html, file_modified_time, file_modified_shift_date = download_file()
    ...
    # ── حفظ وقت/تاريخ استقبال الإيميل في كل رحلة لاستخدامه كمصدر الحقيقة ──
    if file_modified_time or file_modified_shift_date:
        for f in flights:
            if file_modified_time:
                f["email_time"] = file_modified_time
            if file_modified_shift_date:
                f["date"] = file_modified_shift_date
        print(
            f"  [email_time] Set from file Last-Modified: time={file_modified_time or '-'} "
            f"date={file_modified_shift_date or '-'}"
        )
    ...
    if os.environ.get("AIRLABS_API_KEY", "").strip():
        ...
        for f in flights:
            ...
            if not flt:
                continue
            flight_date_iso = file_modified_shift_date or get_shift_date(now)
            info = fetch_flight_info_airlabs(flt, flight_date=flight_date_iso, dep_iata="MCT", arr_iata=(f.get("destination") or "").strip().upper() or None)
            if info:
                std  = (info.get("std") or "").strip()
                etd  = (info.get("etd") or "").strip()
                if std and etd and std != etd:
                ...
                if dest:
                    f["destination"] = dest
                enriched += 1
        print(f"Enriched {enriched} flight(s) via AirLabs using email received date: {file_modified_shift_date or get_shift_date(now)}.")
    else:
