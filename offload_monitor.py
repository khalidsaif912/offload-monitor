...
def fetch_std_etd_airlabs(flight_iata: str, *, flight_date: str | None = None, dep_iata: str | None = None, arr_iata: str | None = None) -> str:
    """Fetch STD/ETD from AirLabs (free-tier possible) if AIRLABS_API_KEY is set.

    Returns a compact string like:
      - 'STD 14:30'
      - 'STD 14:30 · ETD 14:55'
    If no data or API key missing, returns empty string.
    """
    ...
    # Pick the best matching schedule row.
    # AirLabs may return multiple rows for the same flight number (different days / codeshares).
    wanted_date = (flight_date or "").strip()
    row = None

    def _row_matches_date(r: dict) -> bool:
        if not wanted_date:
            return True
        # Try a few common fields that may contain a date.
        for key in ("dep_time", "dep_time_utc", "dep_scheduled", "dep_estimated"):
            v = str(r.get(key, "") or "")
            if wanted_date and wanted_date in v:
                return True
        return False
    ...
    for rrow in rows:
        if not isinstance(rrow, dict):
            continue
        if str(rrow.get("flight_iata", "") or "").strip().upper() != flight_iata:
            continue
        if _row_matches_date(rrow):
            row = rrow
            break

    if row is None:
        # Fallback: first row that matches date, else first row.
        for rrow in rows:
            if isinstance(rrow, dict) and _row_matches_date(rrow):
                row = rrow
                break
    if row is None:
        row = rows[0] if isinstance(rows[0], dict) else {}
    std = row.get("dep_time") or ""
    etd = row.get("dep_estimated") or ""
    std = str(std).strip()
    etd = str(etd).strip()

    if std and etd and std != etd:
        return f"STD {std} · ETD {etd}"
    if std:
        return f"STD {std}"
    return ""
