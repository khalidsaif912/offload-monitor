...
def _render_offload_table(flights: list[dict], meta: dict) -> str:
    """Render the offload table. One row per shipment across all flights."""
    rows_html = ""
    row_num = 0
    for flight in flights:
        flt     = flight.get("flight", "") or "—"
        ...
        for it in items:
            ...
            reason_badge = (
                f'<span style="display:inline-block;padding:2px 8px;border-radius:4px;'                f'background:#fff3e0;border:1px solid #ffb74d;color:#e65100;'                f'font-size:10.5px;font-weight:700;white-space:nowrap;">'                f'{reason.upper()}</span>'
            ) if reason.strip() else "—"

            rows_html += f"""
      <tr style="background:{bg};">
        <td style="padding:8px 6px;border:1px solid #d0d9ee;text-align:center;font-weight:700;">{row_num}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;font-weight:700;color:#0b3a78;">{flt}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;">{date}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;">{std}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;">{etd}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;font-weight:700;">{dest}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;font-family:'Courier New',monospace;font-size:11px;">{awb}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;text-align:center;font-weight:700;">{pcs}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;text-align:center;">{kgs}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;text-align:center;">{priority}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;">{desc}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;text-align:center;">{uld}</td>
        <td style="padding:8px 6px;border:1px solid #d0d9ee;">{reason_badge}</td>
      </tr>"""

    if not rows_html:
        rows_html = """
      <tr><td colspan="13" style="padding:14px 6px;border:1px solid #d0d9ee;
          color:#64748b;text-align:center;font-family:Calibri,Arial,sans-serif;font-weight:600;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr style="background-color:#0b3a78;">
        ...
        <td style="padding:7px 6px;color:#fff;font-weight:700;border:1px solid #0a3166;">ETD</td>
        ...
        <td style="padding:7px 6px;color:#fff;font-weight:700;border:1px solid #0a3166;">KGS</td>
        <td style="padding:7px 6px;color:#fff;font-weight:700;border:1px solid #0a3166;">Priority</td>
...
def _render_manpower_section(roster: dict) -> str:
    """Render Section 6 MANPOWER with full list + sections B-G."""
    import re as _re
    on_duty = roster.get("on_duty", [])
    ...
    def _fmt(emp: dict) -> str:
        """Handle two formats:
           1) Normal:   name field + sn field  → SN 12345 — Name (Dept)
           2) Embedded: 'Name - 12345 (note)'  → SN 12345 — Name (note) (Dept)
        """
        raw  = emp.get("name", "").strip()
        dept = emp.get("dept", "").strip()
        ...
        if m:
            ...
            dept_html = f' <em style="color:#64748b;">({dept})</em>' if dept else ""
            return f"<li>SN {sn_part} — {name_part}{note_html}{dept_html}</li>"

        sn_html   = f"SN {sn} — " if sn else ""
        dept_html = f' <em style="color:#64748b;">({dept})</em>' if dept else ""
        return f"<li>{sn_html}{raw}{dept_html}</li>"

    # القائمة الكاملة (بدون عنوان)
    all_li = "\n".join(_fmt(e) for e in on_duty) if on_duty else '<li style="color:#64748b;">No roster data available for this date.</li>'
    all_staff_html = f'<ul style="{ul_style}">{all_li}</ul>'

    # C) Support Team
    sup_li = "\n".join(
        _fmt(e) for e in on_duty
        if "support" in (e.get("name","") + e.get("dept","")).lower()
    )

    section_b = f'<strong style="{hdr_style}">B) CTU Staff On Duty:</strong><ul style="{ul_style}">{nil_item}</ul>'
    section_c = f'<strong style="{hdr_style}">C) Support Team:</strong><ul style="{ul_style}">{sup_li if sup_li else nil_item}</ul>'
    section_d = f'<strong style="{hdr_style}">D) Sick Leave / No Show / Others:</strong><ul style="{ul_style}">{nil_item}</ul>'
    section_e = f'<strong style="{hdr_style}">E) Annual Leave / Course / Off in Lieu:</strong><ul style="{ul_style}">{nil_item}</ul>'
    section_f = f'<strong style="{hdr_style}">F) Trainee:</strong><ul style="{ul_style}">{nil_item}</ul>'
    section_g = f'<strong style="{hdr_style}">G) Overtime Justification:</strong><ul style="{ul_style}">{nil_item}</ul>'
...
def build_nil_shift_report(date_dir: str, shift: str, now) -> None:
    """Build an empty NIL shift report for days with no offload data."""
    shift_labels = {
        "shift1": {"ar": "صباح",     "en": "Morning",   "time": "06:00 – 15:00"},
        "shift2": {"ar": "ظهر/مساء", "en": "Afternoon", "time": "15:00 – 22:00"},
        "shift3": {"ar": "ليل",      "en": "Night",      "time": "22:00 – 06:00"},
    ...
    roster        = fetch_roster_staff(date_dir, shift)
    manpower_cols = _render_manpower_section(roster)
    nil_table     = _render_offload_table([], {})
...
<body style="margin:0;padding:0;background-color:#eef1f7;font-family:Calibri,Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#eef1f7;">
...
<table width="680" cellpadding="0" cellspacing="0" border="0"
  ...
  <!-- Header -->
  <tr>
    <td style="background:linear-gradient(135deg,#0b3a78,#1a5fb4);padding:18px 24px;">
      <table width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
          <td>
            <div style="font-family:Calibri,Arial,sans-serif;font-size:18px;font-weight:700;
                        color:#ffffff;letter-spacing:.5px;">
              ✈ EXPORT WAREHOUSE ACTIVITY REPORT
            </div>
            <div style="font-family:Calibri,Arial,sans-serif;font-size:13px;
                        color:#bcd4f0;margin-top:4px;">
              {shift_label} &nbsp;·&nbsp; {date_display}
            </div>
          </td>
          <td align="right">
            <div style="background:rgba(255,255,255,.15);border-radius:8px;
                        padding:8px 16px;text-align:center;">
              <div style="font-family:Calibri,Arial,sans-serif;font-size:11px;
                          color:rgba(255,255,255,.75);">Total Offload</div>
              <div style="font-family:Calibri,Arial,sans-serif;font-size:22px;
                          font-weight:800;color:#ffffff;">NIL</div>
            </div>
          </td>
        </tr>
      </table>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      <tr>
        ...
        <td style="padding:6px 10px;background-color:#fff7ed;">
          <span style="font-family:Calibri,Arial,sans-serif;font-size:12px;
                       font-weight:700;color:#c2410c;letter-spacing:1px;">
            4.&nbsp; OFFLOAD
          </span>
        </td>
      </tr>
  ...
  <tr><td style="padding:14px 24px 0 24px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      <tr>
        ...
        <td style="padding:6px 10px;background-color:#eef3fc;">
          <span style="font-family:Calibri,Arial,sans-serif;font-size:12px;
                       font-weight:700;color:#0b3a78;letter-spacing:1px;">
            6.&nbsp; MANPOWER
          </span>
        </td>
      </tr>
  ...
  <tr>
    ...
    # فقط اكتب إذا الملف غير موجود (لا تستبدل التقارير الحقيقية)
    if not out_file.exists():
        out_file.write_text(html, encoding="utf-8")
        print(f"  [NIL report] Created: {date_dir}/{shift}")
...
def build_root_index(now: datetime) -> None:
    ...
    # بناء تقارير NIL لكل الأيام التي ليس لها تقارير بعد
    for day in day_dirs:
