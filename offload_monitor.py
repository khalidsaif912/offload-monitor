...
def fetch_flight_info_flightradar(
    ...
    *,
    ...
    print(f"  [Flightradar] {flight_iata} → dest={dest!r}, std={std!r}, etd={etd!r}")
    return {"std": std, "etd": etd, "dest": dest}
...
def _render_offload_table(flights: list[dict], meta: dict) -> str:
    ...
    def _format_full_date(raw: str) -> str:
        """Convert date like '11MAR', '11MAR26', '11-MAR-26', '2026-03-11' to 'DD-MMM-YY' uppercase."""
        raw = raw.strip()
        if not raw:
            return "—"
        ...
        if m:
            try:
                ...
                pass
        return raw.upper()  # fallback: return as-is uppercase

    def _to_muscat_time(time_str: str) -> str:
        """Ensure time string is in Muscat timezone (UTC+4). If already HH:MM, return as-is."""
        s = (time_str or "").strip()
        if not s:
            return ""
        # If it's an ISO datetime with timezone info, convert to Muscat
        try:
...
def build_shift_report(date_dir: str, shift: str) -> None:
    ...
    for f in flights:
        ...
        needs_std = not (f.get("std_etd") or "").strip()
        needs_dest = not (f.get("destination") or "").strip()
        if not (needs_std or needs_dest):
            continue
        ...
        try:
            ...
            if needs_std:
                if std and etd and std != etd:
                    ...
                    print(f"  [⚠ enrich] {flt}: STD/ETD لا يزال فارغاً رغم البحث في جميع المصادر")

            if needs_dest and dest:
                f["destination"] = dest
            elif needs_dest and not dest:
                print(f"  [⚠ enrich] {flt}: DEST لا يزال فارغاً رغم البحث في جميع المصادر")

            if (f.get("std_etd") or "").strip() or (f.get("destination") or "").strip():
                enriched_count += 1
