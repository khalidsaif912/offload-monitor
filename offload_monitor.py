...
def _render_manpower_section(roster: dict) -> str:
    ...
    # الأقسام التي تُكتب يدوياً — تظهر كعنوان فارغ بدون موظفين
    MANUAL_DEPTS = {"supervisors"}

    grouped_html = ""
    # أولاً: أقسام يدوية (فارغة)
    manual_shown = set()
    for dept in MANUAL_DEPTS:
        if any(e.get("dept","").strip().lower() == dept for e in on_duty):
            pass  # يوجد في roster — سيظهر أدناه
        else:
            # ما في supervisor في roster — اعرض العنوان فارغاً للكتابة يدوياً
            grouped_html += f"""
      <div style="{dept_hdr}">Supervisors:</div>
      <ul style="{ul_style}"><li>&nbsp;</li></ul>"""
            manual_shown.add("supervisors")

    # ثانياً: باقي الأقسام من roster
    for dept, emps in by_dept.items():
