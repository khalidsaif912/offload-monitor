...
def _render_offload_table(flights: list[dict], meta: dict) -> str:
    ...
    for flight in flights:
        ...
        # Trolley/ULD: prefer trolley field, fallback to AWB numbers
        uld_parts = []
        for it in real_items:
            u = (it.get("trolley", "") or "").strip().upper()
            if u and u not in uld_parts:
                uld_parts.append(u)
        # If no trolley fields, use AWB numbers as ULD identifiers
        if not uld_parts:
            for it in real_items:
                a = (it.get("awb", "") or "").strip()
                if a and a not in uld_parts:
                    uld_parts.append(a)
