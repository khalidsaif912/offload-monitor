...
Latest update of same flight replaces previous (Flight+Date+Dest).
"""
...
def parse_offload_html(html):
    """
    Returns list of flights (deduped): latest occurrence of same Flight+Date+Dest wins.
    Each flight: {flight,date,destination,shipments:[{awb,pcs,kgs,desc,reason}]}
    """
    soup = BeautifulSoup(html, "html.parser")

    flights_map = {}   # key -> flight dict (latest overwrites)
    order = []         # keep display order; updated flights move to end
    current = None
    ...
    for row in soup.find_all("tr"):
        ...
        # Flight header row pattern in source HTML:
        # FLIGHT | <flight> | DATE | <date> | DESTINATION | <dest>
        if len(texts) >= 6 and upper and "FLIGHT" in upper[0]:
            ...
            # overwrite old flight with latest snapshot
            current = {
                "flight": flight,
                "date": date,
                "destination": dest,
        ...
        # Skip headers/totals in source HTML
        ...
        # Shipment rows in source HTML: 5 columns (AWB, PCS, KGS, DESC, REASON)
...
# ------------------------------------------------------------
# 12 columns (final table):
#  1=ITEM  2=DATE  3=FLIGHT  4=STD/ATD  5=DEST
#  6=Email  7=Physical  8=Trolley  9=CMS  10=Pieces(Verification)  11=Reason  12=Remarks
NCOLS = 12

def build_table_body(flights):
    """
    IMPORTANT:
    - No repeating column header here.
    - Per flight: blue header row, then shipment rows only.
    """
    html = ""
    idx  = 0
    ...
    for fl in flights:
        ...
        for i, s in enumerate(ships):
            ...
            # verification shown once per flight (first shipment row)
            ...
            html += (
                ...
                f'<td style="padding:8px;border:1px solid #d0d5e8;"></td>'  # STD/ATD manual
                f'<td style="padding:8px;border:1px solid #d0d5e8;font-weight:700;">{dest}</td>'
                f'<td style="padding:8px;border:1px solid #d0d5e8;"></td>'  # Email time manual
                f'<td style="padding:8px;border:1px solid #d0d5e8;"></td>'  # Physical manual
                f'<td style="padding:8px;border:1px solid #d0d5e8;"></td>'  # Trolley manual
                f'<td style="padding:8px;border:1px solid #d0d5e8;"></td>'  # CMS manual
...
def build_report_html(flights, generated_at_local, shift_label):
    ...
    return (
        ...
        # ✅ Column headers -- single row, never repeated (your exact list)
        '<tr>'
        f'<th {TH}>ITEM</th>'
        f'<th {TH}>DATE</th>'
        f'<th {TH}>FLIGHT</th>'
        f'<th {TH}>STD/<br>ETD</th>'
        f'<th {TH}>DEST</th>'
        f'<th {TH}>Email Received Time</th>'
        f'<th {TH}>Physical Cargo<br>received<br>from Ramp</th>'
        f'<th {TH}>Trolley/ ULD Number</th>'
        f'<th {TH}>Offloading Process Completed in CMS</th>'
        f'<th {TH}>Offloading Pieces Verification</th>'
        f'<th {TH}>Offloading Reason</th>'
        f'<th {TH}>Remarks/Additional Information</th>'
        '</tr>'
        ...
        'الأعمدة STD/ETD و Trolley/ULD و CMS و Verification و Email تُعبأ يدوياً.'
        '</div></div>'
    )
