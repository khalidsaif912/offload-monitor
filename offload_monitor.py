...
<body>
    ...
    <div class="wrap">
        {days_html}
        <div class="footer">Generated automatically by GitHub Actions · {today} · Last updated: {last_updated}</div>
    </div>
</body>
</html>"""
...
def main() -> None:
    ...
    # ── فلترة: احتفظ فقط برحلات اليوم ──
    today_iso = now.date().isoformat()
    flights_today = [
        f for f in flights
        if normalize_flight_date(f.get("date", ""), now) == today_iso
    ]
    if flights_today:
        print(f"Filtered to today ({today_iso}): {len(flights_today)}/{len(flights)} flight(s).")
