...
def _render_manpower_section(roster: dict, supervisor_display: str = "") -> str:
    ...
    def _fmt_name(emp):
        ...
        if m:
            ...
            return (
                f'<span style="display:inline-block;width:200px;vertical-align:top;">{name_part}</span>'
                f'<span style="color:#0b3a78;font-weight:700;">{sn_part}</span>'
                f'{note_html}'
            )
        sn_html = (
            f'<span style="display:inline-block;width:200px;vertical-align:top;">{raw}</span>'
            f'<span style="color:#0b3a78;font-weight:700;">{sn}</span>'
        ) if sn else raw
        return sn_html
    ...
    def _fmt_emp_row(name, sn):
        return (
            ...
            f'<span style="display:inline-block;width:200px;vertical-align:top;">{name}</span>'
            f'<span style="color:#0b3a78;font-weight:700;">{sn}</span>'
            f'</li>'
        )
...
/* ══════════════════════════════════════════════════════
   SMART AUTOCOMPLETE — Load Plan / Advance Loading / Offload Table
   ══════════════════════════════════════════════════════ */
(function(){{
  ...
  function normFlight(raw){{
    return raw.trim().toUpperCase().replace(/\s+/g,'');
  }}

  /* ── ghost suggestion UI ── */
  function showGhost(el, ghost){{
    var existing = el.parentElement.querySelector('.ghost-tip');
    if(existing) existing.remove();
    if(!ghost) return;
    var span = document.createElement('span');
    span.className = 'ghost-tip';
    span.textContent = ghost;
    span.style.cssText = 'color:#aaa;font-style:italic;font-size:11px;display:block;margin-top:2px;pointer-events:none;user-select:none;';
    el.parentElement.insertBefore(span, el.nextSibling);
  }}

  function removeGhost(el){{
    var g = el.parentElement && el.parentElement.querySelector('.ghost-tip');
    if(g) g.remove();
  }}

  /* ── fetch flight info from AirLabs ── */
  function fetchFlightInfo(fltIata, date, cb){{
    var apiKey = AIRLABS_KEY;
    if(!apiKey){{ cb(null); return; }}
    var url = 'https://airlabs.co/api/v9/schedules?api_key='+apiKey+'&flight_iata='+fltIata+'&dep_iata=MCT';
    ...
    fetch(url)
      ...
      .then(function(j){{
        ...
        var row = rows[0];
        var std  = (row.dep_scheduled||'').slice(11,16) || '';
        var dest = row.arr_iata || '';
        cb({{std:std, dest:dest}});
      }})
      .catch(function(){{ cb(null); }});
  }}

  /* ════════════════════════════════════════
     A) Load Plan & Advance Loading — أحرف كبيرة + إكمال الرحلة
     ════════════════════════════════════════ */
  function setupFlightListItem(li){{
    li.addEventListener('input', function(){{
      ...
      var upped = old.toUpperCase();
      if(old !== upped){{
        li.innerText = upped;
        /* أعد cursor */
        try{{
          var range = document.createRange();
          var node  = li.firstChild || li;
      ...
      var txt  = li.innerText.trim().toUpperCase().replace(/\s+/g,'');
      var m    = txt.match(/^([A-Z]{{2,3}})(\d{{1,4}})$/);
      if(!m){{ removeGhost(li); return; }}

      var flt  = m[1]+m[2];
      var date = todayMCT();
      /* ghost أولي */
      showGhost(li, flt+'/'+date+'/??? — fetching…');

      fetchFlightInfo(flt, todayISO(), function(info){{
        if(!info){{
          showGhost(li, flt+'/'+date+' — Completed as per plan');
          li.dataset.ghostVal = flt+'/'+date+' \u2014 Completed as per plan';
          return;
        }}
        var dest = info.dest || '???';
        var sug  = flt+'/'+date+'/'+dest+' \u2014 Completed as per plan';
        showGhost(li, sug);
        li.dataset.ghostVal = sug;
      }});
    }});
  ...
  /* Patch addListItem ليربط setupFlightListItem للعناصر الجديدة */
  ...
  /* ════════════════════════════════════════
     B) جدول الأوفلود — date ghost + flight autocomplete
     ════════════════════════════════════════ */
  function setupTableCell(td){{
    ...
    if(col === 'date'){{
      /* عند focus أظهر تاريخ اليوم كـ ghost */
      td.addEventListener('focus', function(){{
        var txt = td.innerText.replace(/\u00a0/g,'').trim();
        if(!txt){{
          showGhost(td, todayMCT()+' (Tab to confirm)');
          td.dataset.ghostVal = todayMCT();
        }}
      }});
      td.addEventListener('keydown', function(e){{
        if(e.key === 'Tab' && td.dataset.ghostVal && !td.innerText.replace(/\u00a0/g,'').trim()){{
          /* لا نمنع Tab الطبيعي — نضع القيمة أولاً */
          td.innerText = td.dataset.ghostVal;
          td.dataset.ghostVal = '';
          removeGhost(td);
          if(typeof triggerAutosave === 'function') triggerAutosave();
        }}
      ...
      td.addEventListener('input', function(){{
        ...
        if(old !== old.toUpperCase()){{
          td.innerText = old.toUpperCase();
          try{{
            var rng = document.createRange();
            var nd  = td.firstChild || td;
      ...
      td.addEventListener('blur', function(){{ removeGhost(td); td.dataset.ghostVal=''; }});
    }}
    ...
    if(col === 'flight'){{
      td.addEventListener('input', function(){{
        ...
        var off = sel.rangeCount ? sel.getRangeAt(0).startOffset : 0;
        var old = td.innerText;
        var up  = old.toUpperCase().replace(/\s+/g,'');
        if(old.replace(/\s+/g,'') !== up){{
          td.innerText = up;
          try{{
            var rng = document.createRange();
            var nd  = td.firstChild || td;
        ...
        var flt = up.replace(/\u00a0/g,'').trim();
        if(!FLT_RE.test(flt)){{ return; }}

        /* ابحث عن خلية التاريخ في نفس الصف */
        ...
        var dateVal  = dateCell ? dateCell.innerText.replace(/\u00a0/g,'').trim() : '';
        /* حوّل 10APR → 2026-04-10 */
        var isoDate  = todayISO();
        if(dateVal){{
          try{{
            var mons = {{JAN:1,FEB:2,MAR:3,APR:4,MAY:5,JUN:6,JUL:7,AUG:8,SEP:9,OCT:10,NOV:11,DEC:12}};
            var dm = dateVal.match(/^(\d{{1,2}})([A-Z]{{3}})(\d{{2,4}})?$/i);
            if(dm){{
              var yr = dm[3] ? (dm[3].length===2 ? 2000+parseInt(dm[3]) : parseInt(dm[3])) : new Date().getFullYear();
              isoDate = yr+'-'+(mons[dm[2].toUpperCase()]||1).toString().padStart(2,'0')+'-'+dm[1].padStart(2,'0');
            }}
          }}catch(ex){{}}
        }}
        ...
        fetchFlightInfo(flt, isoDate, function(info){{
          if(!info) return;
          /* ملء STD/ETD و DEST تلقائياً إذا فارغين */
          if(stdCell && !stdCell.innerText.replace(/\u00a0/g,'').trim() && info.std){{
            stdCell.innerText = info.std;
          }}
  ...
  /* ربط خلايا الجدول الموجودة */
  document.querySelectorAll('[data-col]').forEach(setupTableCell);

  /* ════════════════════════════════════════
     C) D) Sick Leave — autocomplete من قائمة الموظفين
     ════════════════════════════════════════ */
  function setupSickLeave(){{
    var ul = document.getElementById('ul-sickleave');
    if(!ul) return;

    /* اجمع كل الأسماء المرئية في MANPOWER */
    function getAllStaffNames(){{
      var names = [];
      document.querySelectorAll('.mp-list li, #ul-supervisors li, #ul-ctu li, #ul-inventory li, #ul-support li, #ul-trainee li').forEach(function(li){{
        var txt = li.innerText.replace(/\u00a0/g,' ').trim();
        /* استخرج الاسم — قبل الرقم الوظيفي */
        var m = txt.match(/^([A-Za-z\s]+)/);
        var name = m ? m[1].trim() : txt;
        if(name && name.length > 2) names.push(name);
      }});
      return names;
    }}

    function attachSickAutocomplete(li){{
      var dropdown = null;
      ...
      li.addEventListener('input', function(){{
        ...
        var names = getAllStaffNames().filter(function(n){{
          return n.toLowerCase().indexOf(txt) === 0;
        }});
        if(!names.length) return;

        dropdown = document.createElement('div');
        dropdown.style.cssText = 'position:absolute;background:#fff;border:1px solid #0b3a78;border-radius:6px;box-shadow:0 4px 16px rgba(0,0,0,.15);z-index:9999;min-width:220px;max-height:180px;overflow-y:auto;font-size:13px;';

        names.forEach(function(name){{
          var opt = document.createElement('div');
          opt.textContent = name;
          opt.style.cssText = 'padding:7px 12px;cursor:pointer;color:#1b1f2a;';
        ...
        /* ضع الـ dropdown تحت الـ li */
        li.style.position = 'relative';
        li.appendChild(dropdown);
      }});

      li.addEventListener('blur', function(){{
        setTimeout(function(){{ if(dropdown){{ dropdown.remove(); dropdown=null; }} }}, 200);
      }});
    }}

    ul.querySelectorAll('li').forEach(attachSickAutocomplete);

    /* مراقبة إضافة عناصر جديدة */
    var mo = new MutationObserver(function(muts){{
      muts.forEach(function(m){{
        m.addedNodes.forEach(function(n){{
          if(n.tagName==='LI') attachSickAutocomplete(n);
        }});
      }});
    }});
