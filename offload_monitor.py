...
def _render_manpower_section(roster: dict, supervisor_display: str = "") -> str:
    ...
    grouped_html = ""
    # أولاً: قسم Supervisors — يُعرض دائماً في الأعلى مع الاسم المُحدَّد
    sup_in_roster = [e for e in on_duty if e.get("dept","").strip().lower() == "supervisors"]
    if sup_in_roster:
        # يوجد في الروستر — اعرضهم، لكن إذا عندنا اسم بديل نضعه أيضاً
        sup_li_roster = "".join(f"<li>{_fmt_name(e)}</li>\n" for e in sup_in_roster)
        if supervisor_display and supervisor_display not in [e.get("name","") for e in sup_in_roster]:
            # بديل لم يكن في roster الأصلي — أضفه
      ...
      <ul style="{ul_style}">{sup_li_roster}</ul>"""
        ...
        # لا يوجد في roster لكن عندنا اسم (primary أو backup)
        grouped_html += f"""
      <div style="{dept_hdr}">Supervisors:</div>
      <ul style="{ul_style}"><li>{supervisor_display}</li></ul>"""
    else:
...
<table width="760" cellpadding="0" cellspacing="0" border="0"
  ...
  <!-- ═══ FOOTER / SIGNATURE ═══ -->
  <tr>
    <td style="padding:20px 24px 22px 24px; background-color:#f8faff; border-top:2px solid #0b3a78;">
      <table width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
          <td valign="top">
            <!-- Signature block -->
            <div style="font-family:Calibri,Arial,sans-serif; font-size:13px; color:#1b1f2a; line-height:1.8;">
              Best Regards,<br>
              <strong style="font-size:14px; color:#0b3a78;">{supervisor_display}</strong><br>
              <strong style="font-size:13px; color:#1b1f2a;">Supervisor – Export Operation</strong><br>
              M: 97297474<br>
              <a href="http://www.omansats.om" style="color:#0b3a78; text-decoration:underline;">www.omansats.om</a><br>
              <strong style="color:#1b1f2a;">Oman SATS LLC</strong>
            </div>
            <!-- TRANSOM Cargo logo text + certifications bar -->
            <div style="margin-top:10px; padding:8px 10px; background-color:#ffffff; border:1px solid #d0d5e8; display:inline-block; border-radius:4px;">
              <div style="font-family:Arial,sans-serif; font-size:20px; font-weight:900; color:#cc0000; letter-spacing:1px; line-height:1;">
                TRANSOM
              </div>
              <div style="font-family:Arial,sans-serif; font-size:10px; font-weight:700; color:#555; letter-spacing:2px; margin-top:1px;">
                Cargo
              </div>
              <!-- Certifications row -->
              <div style="margin-top:8px; display:flex; align-items:center; gap:6px; flex-wrap:wrap;">
                <span style="font-family:Arial,sans-serif; font-size:9px; font-weight:700; color:#fff;
                             background:#006400; border-radius:50%; width:30px; height:30px;
                             display:inline-flex; align-items:center; justify-content:center;
                             text-align:center; line-height:1.1;">IATA<br>Pharma</span>
                <span style="font-family:Arial,sans-serif; font-size:9px; font-weight:700; color:#fff;
                             background:#228B22; border-radius:50%; width:30px; height:30px;
                             display:inline-flex; align-items:center; justify-content:center;
                             text-align:center; line-height:1.1;">IATA<br>Fresh</span>
                <span style="font-family:Arial,sans-serif; font-size:9px; font-weight:700; color:#fff;
                             background:#004d99; border-radius:50%; width:30px; height:30px;
                             display:inline-flex; align-items:center; justify-content:center;
                             text-align:center; line-height:1.1;">ISO<br>9001</span>
                <span style="font-family:Arial,sans-serif; font-size:9px; font-weight:700; color:#fff;
                             background:#004d99; border-radius:50%; width:30px; height:30px;
                             display:inline-flex; align-items:center; justify-content:center;
                             text-align:center; line-height:1.1;">ISO<br>22000</span>
                <span style="font-family:Arial,sans-serif; font-size:9px; font-weight:700; color:#fff;
                             background:#5a2d82; border-radius:50%; width:30px; height:30px;
                             display:inline-flex; align-items:center; justify-content:center;
                             text-align:center; line-height:1.1;">HACCP</span>
                <span style="font-family:Arial,sans-serif; font-size:9px; font-weight:700; color:#fff;
                             background:#336699; border-radius:50%; width:30px; height:30px;
                             display:inline-flex; align-items:center; justify-content:center;
                             text-align:center; line-height:1.1;">GDP</span>
                <span style="font-family:Arial,sans-serif; font-size:9px; font-weight:700; color:#fff;
                             background:#cc0000; border-radius:4px; padding:2px 6px;
                             display:inline-flex; align-items:center; justify-content:center;
                             text-align:center; line-height:1.3;">RA3<br><span style="font-size:7px;font-weight:400;">Regulated Agent<br>Status in 3rd Country</span></span>
              </div>
            </div>
          </td>
...
def should_send_email(now, shift: str) -> bool:
    """نافذة الإرسال: ساعة قبل نهاية المناوبة ±10 دقائق."""
    send_times = {
        "shift1": (14, 0),   # Morning تنتهي 15:00
        "shift2": (21, 0),   # Afternoon تنتهي 22:00
        "shift3": (5,  0),   # Night تنتهي 06:00
    }
    target = send_times.get(shift)
    if not target:
        return False
    current = now.hour * 60 + now.minute
    target_m = target[0] * 60 + target[1]
    return abs(current - target_m) <= 10
...
def main() -> None:
    ...
    if STATE_FILE.exists():
        ...
        if old_hash == new_hash and not FORCE_REBUILD:
            ...
            # ── إرسال البريد قبل ساعة من نهاية المناوبة (حتى لو لا يوجد تغيير) ──
            if os.environ.get("EMAIL_SENDER", "").strip():
                today_str = now.strftime("%Y-%m-%d")
                for _shift in ("shift1", "shift2", "shift3"):
                    if should_send_email(now, _shift):
                        print(f"[email] Sending {_shift} report for {today_str}…")
                        send_shift_report_email(today_str, _shift)
            return
    ...
    # ── إرسال البريد قبل ساعة من نهاية المناوبة ──
    if os.environ.get("EMAIL_SENDER", "").strip():
        today_str = now.strftime("%Y-%m-%d")
        for _shift in ("shift1", "shift2", "shift3"):
            if should_send_email(now, _shift):
