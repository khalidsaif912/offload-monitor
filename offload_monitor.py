  ...
  <button id="btn-copy"
    onclick="(function(){{
      ...
      if(navigator.clipboard && window.ClipboardItem){{
        var item = new ClipboardItem({{'text/html': new Blob([full], {{type:'text/html'}}), 'text/plain': new Blob([el.innerText], {{type:'text/plain'}})}});
        ...
        var ta=document.createElement('textarea'); ta.value=el.innerText;
...
(function(){{
  ...
  window.openRecipientsFile = function(){{
    if(!askSendSecret()) return;
    window.open('https://github.com/khalidsaif912/offload-monitor/edit/main/docs/data/email_recipients.json', '_blank');
  }};

  window.sendEmailNow = function(){{
    if(!askSendSecret()) return;
