...
def fetch_roster_staff(date_dir: str, shift: str) -> dict:
    """Fetch roster staff for the given date/shift using local cached roster.json."""
    data = ensure_local_roster_json(force_refresh=False)

    days = data.get("days", {})
    day_data = days.get(date_dir)

    # إذا اليوم غير موجود، جرّب تحديث الملف من المصدر مرة أخرى
    if not day_data:
        print(f"  [roster] No local data for {date_dir}, refreshing from remote...")
        data = ensure_local_roster_json(force_refresh=True)
        days = data.get("days", {})
        day_data = days.get(date_dir)
    ...
    if not cards_html:
        print(f"  [roster] cards_html empty for {date_dir}")
        return {"on_duty": [], "on_leave": []}
