    ...
    while i < len(all_rows):
        ...
        if ("FLIGHT" in joined and "DATE" in joined and
            ...
            while j < len(all_rows):
                ...
                # ULD/TROLLEY يبدأ دائماً بحروف (مثل AKE1234, PMC5678, BT001, CBT...)
                # بينما AWB كله أرقام فقط.
                # لذلك: أي قيمة تبدأ بحرف → ULD، نربطه بآخر شحنة أو نحفظه كـ pending.
                _is_uld = bool(awb_clean and re.match(r"^[A-Z]", awb_clean))
                if _is_uld and not any([pcs, kgs, desc, rsn]):
                    if items:
                        prev_uld = (items[-1].get("trolley") or "").strip()
                        if prev_uld:
                            items[-1]["trolley"] = prev_uld + ", " + awb
                        else:
                            items[-1]["trolley"] = awb
                    else:
                        if pending_trolley:
                            pending_trolley = pending_trolley + ", " + awb
                        else:
                            pending_trolley = awb
                    j += 1
                    continue
                # إذا كانت AWB تبدأ بحروف لكن معها بيانات أخرى → ULD مع بيانات
                # نحفظ ULD في trolley ونضيف الشحنة بدون AWB
                if _is_uld and any([pcs, kgs, desc, rsn]):
                    trolley = awb
                    awb = ""
                    items.append({
                        "awb":         awb,
...
def _render_offload_table(flights: list[dict], meta: dict) -> str:
    ...
    for flight in flights:
        ...
        # real_items: أي item يحتوي على AWB أو ULD/trolley أو أي بيانات أخرى
        real_items = [i for i in items if any([
            (i.get("awb","") or "").strip(),
            (i.get("trolley","") or "").strip(),
            (i.get("pcs","") or "").strip(),
        ...
        # لكن إذا كانت trolley فارغة وawb يبدأ بحرف → هو ULD
        uld_parts = []
        for it in real_items:
            u = (it.get("trolley", "") or "").strip().upper()
            if not u:
                # fallback: إذا awb يبدأ بحرف فهو ULD وليس AWB
                awb_val = (it.get("awb", "") or "").strip().upper()
                if awb_val and re.match(r"^[A-Z]", awb_val):
                    u = awb_val
