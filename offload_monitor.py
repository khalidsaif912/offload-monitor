...
def _render_offload_table(flights: list[dict], meta: dict) -> str:
    ...
    def _format_full_date(raw: str) -> str:
        """Convert date like '11MAR', '11MAR26', '11-MAR-26', '2026-03-11' to 'DD-Mon-YY'."""
        raw = raw.strip()
        if not raw:
            return "—"
        ...
        for fmt in ("%Y-%m-%d", "%d-%b-%y", "%d-%b-%Y", "%d%b%y", "%d%b%Y"):
            try:
                ...
                pass
        # Try short format like '11MAR' (no year) — use current year
        raw_up = raw.upper()
        m = re.match(r"(\d{1,2})([A-Z]{3})$", raw_up)
        if m:
            try:
                dt = datetime.strptime(f"{m.group(1)}{m.group(2)}{datetime.now().year}", "%d%b%Y")
                return dt.strftime("%d-%b-%y")
            except ValueError:
                pass
    ...
    for flight in flights:
        ...
        # Build one row per flight (not per AWB) matching the vertical format
        item_num += 1
        bg = row_odd if item_num % 2 == 0 else row_even

        # Offloading reason: combine from items if available
        reasons = []
        ...
        # Trolley/ULD: combine from items
        ulds = []
        for it in real_items:
            u = (it.get("trolley", "") or "").strip()
            if u and u not in ulds:
                ulds.append(u)
        uld_display = ", ".join(ulds) if ulds else ""
    ...
    table_html = f"""
    <table width="100%" cellpadding="0" cellspacing="0" border="0" data-offload-table="1"
...
def _update_flight_json(folder: Path, flight: dict) -> None:
    ...
    if file_path.exists():
        try:
            ...
            existing.update({k: v for k, v in flight.items() if v})
...
def build_shift_report(date_dir: str, shift: str) -> None:
    ...
    # ── Enrich STD/ETD from AirLabs if missing ──
    if os.environ.get("AIRLABS_API_KEY", "").strip():
        for f in flights:
            if (f.get("std_etd") or "").strip():
                continue  # already has STD/ETD
            flt = (f.get("flight") or "").strip()
            if not flt:
                continue
            try:
                info = fetch_flight_info_airlabs(flt, flight_date=date_dir, dep_iata="MCT")
                if info:
...
<table width="1100" cellpadding="0" cellspacing="0" border="0" id="report-content"
  ...
  <!-- ═══ HEADER ═══ -->
  <tr>
    <td data-email-banner="1" style="background-color:#0b3a78; padding:0; mso-padding-alt:0;">
      <table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation" style="background-color:#0b3a78;">
        <tr>
          <td width="8" data-email-banner="1" style="background-color:#f59e0b; font-size:1px; line-height:1px; mso-line-height-rule:exactly;">&nbsp;</td>
          <td data-email-banner="1" style="padding:20px 22px 18px 16px; background-color:#0b3a78;">
            <table width="100%" cellpadding="0" cellspacing="0" border="0" role="presentation" style="background-color:#0b3a78;">
              <tr>
                <td data-email-banner="1" style="background-color:#0b3a78;">
                  <div style="font-family:Calibri,Arial,sans-serif; font-size:21px; font-weight:800; color:#ffffff; letter-spacing:0.5px; line-height:1.25;">
                    &#9992;&nbsp; Export Warehouse Activity Report
                  </div>
                  <div style="font-family:Calibri,Arial,sans-serif; font-size:13px; color:#ffffff; margin-top:7px; letter-spacing:0.2px;">
                    Shift Date:&nbsp;<strong style="color:#fde68a; font-weight:700;">{date_display}</strong>
                    &nbsp;&nbsp;|&nbsp;&nbsp;
                    Time:&nbsp;<strong style="color:#fde68a; font-weight:700;">{sl['time']} LT</strong>
                    &nbsp;&nbsp;|&nbsp;&nbsp;
                    <strong style="color:#fde68a; font-weight:700;">{sl['en']} Shift</strong>
                  </div>
                </td>
                <td align="right" valign="middle" data-email-banner="1" style="padding-right:6px; background-color:#0b3a78;">
                  <div style="font-family:Calibri,Arial,sans-serif; font-size:12px; color:#93c5fd; line-height:1.5; text-align:right;">
                    Oman SATS LLC<br>
...
(function(){{
  ...
  function forceReadableContrast(node){{
    ...
    /* Skip banner — intentional dark bg with white text */
    if(node.getAttribute && node.getAttribute('data-email-banner')) return;
    var _p = node.parentElement;
    while(_p){{ if(_p.getAttribute && _p.getAttribute('data-email-banner')) return; _p = _p.parentElement; }}
  ...
  function buildReportHtml(){{
    ...
    /* ── Helper: is node inside the banner? ── */
    function isInBanner(node){{
      var p = node;
      while(p){{ if(p.getAttribute && p.getAttribute('data-email-banner')) return true; p = p.parentElement; }}
      return false;
    }}

    /* ── Step 1: Fix banner — keep dark blue bg + enforce white/amber text ── */
    clone.querySelectorAll('[data-email-banner]').forEach(function(el2){{
      var bg2 = (el2.style.backgroundColor || '').toLowerCase().replace(/\\s+/g,'');
      if(bg2 !== 'rgb(245,158,11)' && bg2 !== '#f59e0b'){{
        el2.style.backgroundColor = '#0b3a78';
        el2.style.background = '#0b3a78';
      }}
      el2.querySelectorAll('div,span,strong,td,p').forEach(function(child){{
        var c = (child.style.color || '').toLowerCase().replace(/\\s+/g,'');
        if(c === '#fde68a' || c === 'rgb(253,230,138)') return; /* keep amber */
        if(c === '#93c5fd' || c === 'rgb(147,197,253)') return; /* keep light-blue sub */
        child.style.color = '#ffffff';
      }});
    }});

    /* ── Step 2: Outlook fix — convert dark backgrounds to light (skip banner) ── */
    ...
    clone.querySelectorAll('td,th,tr,div,span,strong').forEach(function(el2){{
      if(isInBanner(el2)) return; /* skip banner elements */
    ...
    /* ── Strip contenteditable + tabindex — not safe/needed in email ── */
    clone.querySelectorAll('[contenteditable]').forEach(function(el2){{
      el2.removeAttribute('contenteditable');
      el2.style.outline = 'none';
    }});
    clone.querySelectorAll('[tabindex]').forEach(function(el2){{
      el2.removeAttribute('tabindex');
    }});

    /* ── Remove back-link row (not useful in email) ── */
    var backRow = clone.querySelector('#back-link-row');
    if(backRow) backRow.style.display = 'none';

    /* ── Fix offload table: ensure empty-only rows don't bleed outside table ── */
    clone.querySelectorAll('table[data-offload-table] tbody tr').forEach(function(row){{
      var cells = row.querySelectorAll('td');
      /* Count cells with real content (not &nbsp; or whitespace only) */
      var hasContent = false;
      cells.forEach(function(cell, idx){{
        if(idx === 0) return; /* skip item number cell */
        var txt = (cell.textContent || cell.innerText || '').replace(/\\u00a0/g,'').trim();
        if(txt) hasContent = true;
      }});
      /* Empty rows: remove contenteditable residue and ensure they stay inside table */
      if(!hasContent){{
