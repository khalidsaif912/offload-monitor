...
def _render_manpower_section(roster: dict) -> str:
    ...
    # الأقسام المستثناة من القائمة الرئيسية
    EXCLUDED_DEPTS = {"officers"}
    EXCLUDED_SNS   = {"990737"}  # Said Al Amri

    def _is_support(e: dict) -> bool:
    ...
    # القائمة الرئيسية: بدون Officers, Supervisors, Support
    main_emps = [e for e in on_duty if not _is_excluded(e)]
    all_li = "\n".join(_fmt(e) for e in main_emps) if main_emps else '<li style="color:#64748b;">No roster data available for this date.</li>'
    all_staff_html = f'<ul style="{ul_style}">{all_li}</ul>'

    # C) Support Team
    sup_li = "\n".join(_fmt(e) for e in on_duty if _is_support(e))
...
def build_nil_shift_report(date_dir: str, shift: str, now) -> None:
    """Build a full NIL shift report for days with no offload data.
    Uses the same full HTML template as build_shift_report."""
    out_dir  = DOCS_DIR / date_dir / shift
    out_file = out_dir / "index.html"
    # لا تستبدل تقارير حقيقية
    if out_file.exists():
        return
    ...
    roster        = fetch_roster_staff(date_dir, shift)
    manpower_cols = _render_manpower_section(roster)
    offload_table_html = _render_offload_table([], {})
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      <tr>
        ...
        <td style="padding:6px 10px; background-color:#fff7ed;">
          <span style="font-family:Calibri,Arial,sans-serif; font-size:12px; font-weight:700; color:#c2410c; letter-spacing:1px;">4.&nbsp; CHECKS &amp; COMPLIANCE</span>
        </td>
      </tr>
    </table>
    <div style="font-family:Calibri,Arial,sans-serif; font-size:13.5px; color:#1b1f2a; line-height:1.7; margin-top:10px; padding:0 4px;">
