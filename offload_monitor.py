...
def fetch_roster_staff(date_dir: str, shift: str) -> dict:
    ...
    for dept_card in soup.select(".deptCard"):
        ...
        for shift_card in dept_card.select("details.shiftCard, .shiftCard"):
            shift_label = _normalize_shift_label(
                shift_card.get("data-shift")
                or cell_text(shift_card.select_one(".shiftLabel"))
                or cell_text(shift_card.select_one("summary"))
            )
...
def build_shift_report(date_dir: str, shift: str) -> None:
    ...
    report_build_id = datetime.now(ZoneInfo(TIMEZONE)).isoformat()
...
window._REPORT_BUILD_ID    = '{report_build_id}';
</script>
...
(function(){{
  ...
  var FLT_RE = /^([A-Z0-9]{{2,3}})(\d{{1,4}}[A-Z]?)$/i;
  ...
  function fetchFlightInfo(flt, isoDate, cb) {{
    ...
    if(local && local.dest && local.std) {{ cb(local); return; }}
        ...
        if(!rows.length) {{
          return fetch('https://airlabs.co/api/v9/flights?api_key='+AIRLABS_KEY
            ...
            .then(function(j2) {{
              ...
              if(!r2rows.length) {{ cb(local||null); return; }}
              doRow(r2rows[0]);
            }});
        }}
        doRow(rows[0]);
    ...
    function doRow(row) {{
      cb({{
        std:  (row.dep_scheduled||row.dep_time||'').slice(11,16),
        etd:  (row.dep_estimated||'').slice(11,16),
  ...
  function setupFlightListItem(li, isLoadPlan) {{
    ...
    /* input: كتابة رقم رحلة → fetch + ghost */
  ...
  function setupManpowerLi(li) {{
    ...
    li.addEventListener('input', function() {{
      ...
      document.querySelectorAll('[data-sn]').forEach(function(el) {{
        var sn = el.dataset.sn||'', name = el.dataset.name||'';
    ...
    li.addEventListener('blur', function() {{
      setTimeout(function() {{
        var dd = li.querySelector('.sn-drop');
        if(dd) dd.remove();
      }}, 200);
    }});
  }}

  var MP_IDS = ['ul-supervisors','ul-ctu','ul-inventory','ul-support',
                'ul-sickleave','ul-annualleave','ul-trainee','ul-overtime'];
...
(function(){{
  ...
  var STORE   = 'reports';
  var PAGE_KEY = location.pathname + '::' + (window._REPORT_BUILD_ID || 'v1');
  var _db = null;
  var _saveTimer = null;
  ...
  function _meaningfulHtml(html){{
    var tmp = document.createElement('div');
    tmp.innerHTML = html || '';
    var txt = (tmp.innerText || tmp.textContent || '').replace(/ /g,' ').replace(/\s+/g,' ').trim();
    return !!txt;
  }}
      ...
      req.onsuccess = function(e){{
        ...
        Object.keys(data).forEach(function(k){{
          if(k.indexOf('__ul__') === 0){{
            ...
            if(ul){{
              var currentHasRoster = !!ul.querySelector('[data-sn]');
              var savedHasRoster   = (data[k] || '').indexOf('data-sn=') !== -1;
              var savedMeaningful  = _meaningfulHtml(data[k]);
              if(currentHasRoster && !savedHasRoster && !savedMeaningful) return;
              ul.innerHTML = data[k];
            }}
          }}
        }});
