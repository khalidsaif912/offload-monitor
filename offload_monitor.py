...
def normalize_flight_date(date_str: str, now: datetime) -> str:
    """Convert date strings like:
       27FEB, 27FEB26, 27-FEB, 27-FEB-26, 27 FEB, 27 FEB 2026, 2026-02-27
       into YYYY-MM-DD.
    """
    ...
    # 27 FEB 26
    if day is None:
        m = re.fullmatch(r"(\d{1,2})\s*([A-Z]{3,})\s*(\d{2})", s)
        if m:
            day = int(m.group(1))
            mon = m.group(2)[:3]
    ...
    # 27FEB26
    if day is None:
        m = re.fullmatch(r"(\d{1,2})([A-Z]{3})(\d{2})", s)
        if m:
            day = int(m.group(1))
            mon = m.group(2)
            year = 2000 + int(m.group(3))
...
def build_shift_report(date_dir: str, shift: str) -> None:
    ...
    airlabs_key = os.environ.get("AIRLABS_API_KEY", "").strip()
    print(f"  [debug] AIRLABS_API_KEY exists: {bool(airlabs_key)}")
    ...
    if airlabs_key:
        for f in flights:
            if (f.get("std_etd") or "").strip():
                print(f"  [AirLabs] Skip {f.get('flight','')} - std_etd already exists: {f.get('std_etd','')}")
                continue

            flt = re.sub(r"\s+", "", (f.get("flight") or "").strip().upper())
            raw_date = (f.get("date") or "").strip()
            raw_dest = (f.get("destination") or "").strip().upper()

            if not flt:
                print("  [AirLabs] Skip flight with empty flight number")
                continue

            normalized_date = normalize_flight_date(raw_date, datetime.now(ZoneInfo(TIMEZONE))) if raw_date else date_dir

            print(
                f"  [AirLabs] Trying flight={flt}, "
                f"raw_date={raw_date!r}, normalized_date={normalized_date!r}, "
                f"dest={raw_dest!r}"
            )
            ...
            try:
                info = fetch_flight_info_airlabs(
                    flt,
                    flight_date=normalized_date or date_dir,
                    dep_iata="MCT",
                    arr_iata=raw_dest or None,
                )

                print(f"  [AirLabs] Result for {flt}: {info}")

                if not info:
                    print(f"  [AirLabs] No match found for {flt}")
                    continue

                std = (info.get("std") or "").strip()
                etd = (info.get("etd") or "").strip()
                api_dest = (info.get("dest") or "").strip().upper()
                ...
                if not raw_dest and api_dest:
                    f["destination"] = api_dest

                if f.get("std_etd"):
                    enriched_count += 1
                    _update_flight_json(folder, f)
                    print(f"  [AirLabs] Enriched {flt} with std_etd={f['std_etd']!r}")
                else:
                    print(f"  [AirLabs] Match found for {flt}, but std/etd were empty")

            except Exception as exc:
                print(f"  [report-enrich] {flt}: {exc}")

    print(f"Enriched {enriched_count} flight(s) via AirLabs.")
