...
def _render_manpower_section(roster: dict, supervisor_display: str = "") -> str:
    ...
    def _fmt_name(emp):
        ...
        if m:
            ...
            return (
                f'<span style="display:inline-block;min-width:220px;font-family:\'Courier New\',Courier,monospace;">{name_part}</span>'
                f'<span style="font-family:\'Courier New\',Courier,monospace;color:#0b3a78;font-weight:600;">{sn_part}</span>'
                f'{note_html}'
            )
        sn_html = (
            f'<span style="display:inline-block;min-width:220px;font-family:\'Courier New\',Courier,monospace;">{raw}</span>'
            f'<span style="font-family:\'Courier New\',Courier,monospace;color:#0b3a78;font-weight:600;">{sn}</span>'
        ) if sn else raw
        return sn_html
    ...
    # C) Support Team — fixed staff from roster based on shift
    _support_from_roster = [e for e in on_duty if _is_support(e)]
    # Fixed Inventory staff always present in MANPOWER
    _inventory_staff_items = (
        '<li style="font-family:\'Courier New\',Courier,monospace;list-style:none;padding:2px 0;">'
        '<span style="display:inline-block;min-width:220px;">Mohamed Al Subhi</span>'
        '<span style="color:#0b3a78;font-weight:600;">82592</span>'
        '</li>\n'
        '<li style="font-family:\'Courier New\',Courier,monospace;list-style:none;padding:2px 0;">'
        '<span style="display:inline-block;min-width:220px;">Said Al Amri</span>'
        '<span style="color:#0b3a78;font-weight:600;">990737</span>'
        '</li>'
    )
    # Fixed Support Team C staff
    _support_fixed_items = (
        '<li contenteditable="true" style="outline:none;font-family:\'Courier New\',Courier,monospace;">'
        '<span style="display:inline-block;min-width:220px;">Ahmed Al Sabii</span>'
        '<span style="color:#0b3a78;font-weight:600;">82653</span>'
        '</li>\n'
        '<li contenteditable="true" style="outline:none;font-family:\'Courier New\',Courier,monospace;">'
        '<span style="display:inline-block;min-width:220px;">Jamal Al Hadi</span>'
        '<span style="color:#0b3a78;font-weight:600;">82565</span>'
        '</li>'
    )
    if _support_from_roster:
        _c_items = "".join(
            f'<li contenteditable="true" style="outline:none;">{_fmt_name(e)}</li>'
            for e in _support_from_roster
        ) + "\n" + _support_fixed_items
    else:
        _c_items = _support_fixed_items
    ...
    section_b = (
        ...
        f'<ul id="ul-ctu" style="{ul_style}">{_li_edit}</ul>'
        f'<button onclick="addListItem(\'ul-ctu\')" style="{_btn_style}">+ Add</button>'
    )

    # Inventory section
    _inv_ul_style = "margin:2px 0 10px 0;padding:0;list-style:none;"
    section_inventory = (
        f'<div style="{hdr_style}">Inventory:</div>'
        f'<ul id="ul-inventory" style="{_inv_ul_style}">{_inventory_staff_items}</ul>'
...
function loadManpower() {{
    ...
    fetch(url)
        ...
        .then(function(htmlText) {{
            ...
            // Inventory — ثابت دائماً
            html += '<div class="mp-dept">Inventory</div>';
            html += '<div class="mp-emp"><span class="emp-sn">82592</span><span class="emp-name">Mohamed Al Subhi</span></div>';
            html += '<div class="mp-emp"><span class="emp-sn">990737</span><span class="emp-name">Said Al Amri</span></div>';
            // C) Support Team — ثابت دائماً
            html += '<div class="mp-dept">C) Support Team</div>';
            html += '<div class="mp-emp"><span class="emp-sn">82653</span><span class="emp-name">Ahmed Al Sabii</span></div>';
            html += '<div class="mp-emp"><span class="emp-sn">82565</span><span class="emp-name">Jamal Al Hadi</span></div>';
            el.innerHTML = html;
            mpData = grouped;
        }})
        .catch(function(err) {{
            // Inventory — ثابت دائماً حتى عند الخطأ
            var fallback = '<div class="mp-dept">Inventory</div>';
            fallback += '<div class="mp-emp"><span class="emp-sn">82592</span><span class="emp-name">Mohamed Al Subhi</span></div>';
            fallback += '<div class="mp-emp"><span class="emp-sn">990737</span><span class="emp-name">Said Al Amri</span></div>';
            fallback += '<div class="mp-dept">C) Support Team</div>';
            fallback += '<div class="mp-emp"><span class="emp-sn">82653</span><span class="emp-name">Ahmed Al Sabii</span></div>';
            fallback += '<div class="mp-emp"><span class="emp-sn">82565</span><span class="emp-name">Jamal Al Hadi</span></div>';
